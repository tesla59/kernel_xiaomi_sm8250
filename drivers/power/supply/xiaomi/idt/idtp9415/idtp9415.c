...
static void idtp922x_request_adapter(struct idtp9220_device_info *di);
static void idtp9220_set_charging_param(struct idtp9220_device_info *di);
static int program_fw(struct idtp9220_device_info *di, u16 destAddr, u8 *src, u32 size);
...
static int idtp9220_read(struct idtp9220_device_info *di, u16 reg, u8 *val) {
	unsigned int temp;
	int rc;
...
static int idtp9220_write(struct idtp9220_device_info *di, u16 reg, u8 val) {
	int rc = 0;
...
static int idtp9220_masked_write(struct idtp9220_device_info *di, u16 reg, u8 mask, u8 val)
{
	int rc = 0;
...
static int idtp9220_read_buffer(struct idtp9220_device_info *di, u16 reg, u8 *buf, u32 size) {
	int rc =0;
...
static int idtp9220_write_buffer(struct idtp9220_device_info *di, u16 reg, u8 *buf, u32 size) {
	int rc = 0;
...
static u32 ExtractPacketSize(u8 hdr) {
	if (hdr < 0x20)
		return 1;
	if (hdr < 0x80)
		return (2 + ((hdr - 0x20) >> 4));
...
static void idtp922x_clrInt(struct idtp9220_device_info *di, u8 *buf, u32 size) {
	di->bus.write_buf(di, REG_SYS_INT_CLR, buf, size);
	di->bus.write(di, REG_SSCMND, CLRINT);
}

static void reverse_clrInt(struct idtp9220_device_info *di, u8 *buf, u32 size) {
	di->bus.write_buf(di, REG_SYS_INT_CLR, buf, size);
	di->bus.write(di, REG_TX_CMD, TX_FOD_EN | TX_CLRINT);
}

static void idtp922x_enable_ext5v(struct idtp9220_device_info *di)
{
	dev_info(di->dev, "enable external 5v\n");
	di->bus.write(di, REG_EXTERNAL_5V, BIT(0));
}

static void idtp922x_sendPkt(struct idtp9220_device_info *di, ProPkt_Type *pkt) {
...
static void idtp922x_receivePkt(struct idtp9220_device_info *di, u8 *buf) {
	u8 header;
	int rc;
	u32 size;
...
static void idtp922x_set_adap_vol(struct idtp9220_device_info *di, u16 mv)
{
...
static void idtp922x_set_pmi_icl(struct idtp9220_device_info *di, int mA)
{
...
static void idtp922x_request_uuid(struct idtp9220_device_info *di, int is_epp)
{
	ProPkt_Type pkt;
	pkt.header = PROPRIETARY18;
	if(is_epp)
...
static void idtp922x_get_tx_vin(struct idtp9220_device_info *di)
{
	ProPkt_Type pkt;
	pkt.header = PROPRIETARY18;
	pkt.cmd = BC_READ_Vin;
...
static void idtp922x_retry_cmd(struct idtp9220_device_info *di)
{
	ProPkt_Type pkt;
 ...
 */
static void idtp9220_retry_id_auth(struct idtp9220_device_info *di)
{
	ProPkt_Type pkt;
...
static void idtp922x_sent_vout(struct idtp9220_device_info *di, u8 data_l, u8 data_h)
{
       ProPkt_Type pkt;
       pkt.header = PROPRIETARY38;
...
static void idtp922x_sent_iout(struct idtp9220_device_info *di, u8 data_l, u8 data_h)
{
       ProPkt_Type pkt;
       pkt.header = PROPRIETARY38;
...
static void idtp922x_request_low_addr(struct idtp9220_device_info *di)
{
	ProPkt_Type pkt;
	pkt.header = PROPRIETARY18;
...
static void idtp922x_request_high_addr(struct idtp9220_device_info *di)
{
	ProPkt_Type pkt;
	pkt.header = PROPRIETARY18;
...
static void idtp922x_sent_tx_mac(struct idtp9220_device_info *di)
{
	int64_t ble_mac = 0;
	union power_supply_propval val = {0, };
	uint32_t lens = 6;
...
static void idtp922x_sent_chip_version(struct idtp9220_device_info *di)
{
	ProPkt_Type pkt;
...
static void idtp922x_sent_fw_version(struct idtp9220_device_info *di)
{
	ProPkt_Type pkt;
	u8 fw_app_ver[4];
...
static void idtp9220_bpp_connect_load_work(struct work_struct *work)
{
	...
	bool is_thyme = (socinfo_get_platform_type() == HW_PLATFORM_THYME);
	int icl_max = is_thyme ? THYME_BPP_DEFAULT_CURRENT : BPP_DEFAULT_CURRENT;
#else
...
int idtp9415_op_ble_flag(int en)
{
	int rc;
	if (!g_di)
		return -EINVAL;
...
#ifdef CONFIG_MACH_XIAOMI_SM8250
	if (socinfo_get_platform_type() != HW_PLATFORM_THYME) {

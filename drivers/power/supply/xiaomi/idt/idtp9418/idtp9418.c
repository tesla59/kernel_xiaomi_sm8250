...
static int program_fw(struct idtp9220_device_info *di, u16 destAddr, u8 *src,
	       u32 size);
...
static int idtp9220_masked_write(struct idtp9220_device_info *di, u16 reg, u8 mask,
			  u8 val)
{
	int rc = 0;
...
static int idtp9220_read_buffer(struct idtp9220_device_info *di, u16 reg, u8 *buf,
			 u32 size)
{
	int rc = 0;
...
static int idtp9220_write_buffer(struct idtp9220_device_info *di, u16 reg, u8 *buf,
			  u32 size)
{
	int rc = 0;
...
static void idtp922x_receivePkt2(struct idtp9220_device_info *di, u8 *buf)
{
	int rc;
...
static void idtp922x_sent_pen_mac(struct idtp9220_device_info *di)
{
	int64_t ble_mac = 0;
	union power_supply_propval val = { 0, };
...
static void idtp922x_sent_reverse_cnf(struct idtp9220_device_info *di)
{
	ProPkt_Type pkt;
	pkt.header = PROPRIETARY18;
...
int idtp9418_op_ble_flag(int en)
{
	int rc;
	if (!g_di)
		return -EINVAL;

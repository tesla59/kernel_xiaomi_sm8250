...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
	...
	case PROC_READ_REGISTER:
		writelen = 1;
		ret = fts_i2c_write_mi(client, writebuf + 1, writelen);
		if (ret < 0) {
...
#if FTS_ESDCHECK_EN
	fts_esdcheck_proc_busy_mi(1);
#endif

	switch (ts_data->proc_opmode) {
	case PROC_READ_REGISTER:
		readlen = 1;
		ret = fts_i2c_read_mi(client, NULL, 0, buf, readlen);
		if (ret < 0) {
#if FTS_ESDCHECK_EN
			fts_esdcheck_proc_busy_mi(0);
#endif
...
***********************************************************************/
int fts_create_apk_debug_channel_mi(struct fts_ts_data_mi *ts_data)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
...
* Return:
***********************************************************************/
void fts_release_apk_debug_channel_mi(struct fts_ts_data_mi *ts_data)
{
...
static ssize_t fts_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	fts_esdcheck_proc_busy_mi(1);
#endif
	if (fts_i2c_read_reg_mi(client, FTS_REG_FW_VER, &fwver) < 0) {
	...
	if (rw_op.len < 0) {
		...
		if (RWREG_OP_READ == rw_op.type) {
			if (rw_op.len == 1) {
				u8 reg, val;
				reg = rw_op.reg & 0xFF;
				rw_op.res = fts_i2c_read_reg_mi(client, reg, &val);
				rw_op.val = val;
			} else {
				char reg;
				reg = rw_op.reg & 0xFF;

				rw_op.res = fts_i2c_read_mi(client, &reg, 1, rw_op.opbuf, rw_op.len);
			}
			...
			if (rw_op.len == 1) {
				...
				reg = rw_op.reg & 0xFF;
				val = rw_op.val & 0xFF;
				rw_op.res = fts_i2c_write_reg_mi(client, reg, val);
			} else {
				rw_op.res = fts_i2c_write_mi(client, rw_op.opbuf, rw_op.len);
			}
			if (rw_op.res < 0) {
...
static ssize_t fts_fwupgradebin_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	fts_upgrade_bin_mi(client, fwname, 0);
...
static ssize_t fts_fwforceupg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	fts_upgrade_bin_mi(client, fwname, 1);
...
static ssize_t fts_dumpreg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	fts_esdcheck_proc_busy_mi(1);
#endif
	fts_i2c_read_reg_mi(client, FTS_REG_POWER_MODE, &val);
	count += snprintf(buf + count, PAGE_SIZE, "Power Mode:0x%02x\n", val);

	fts_i2c_read_reg_mi(client, FTS_REG_FW_VER, &val);
	count += snprintf(buf + count, PAGE_SIZE, "FW Ver:0x%02x\n", val);

	fts_i2c_read_reg_mi(client, FTS_REG_LIC_VER, &val);
	count += snprintf(buf + count, PAGE_SIZE, "LCD Initcode Ver:0x%02x\n", val);

	fts_i2c_read_reg_mi(client, FTS_REG_IDE_PARA_VER_ID, &val);
	count += snprintf(buf + count, PAGE_SIZE, "Param Ver:0x%02x\n", val);

	fts_i2c_read_reg_mi(client, FTS_REG_IDE_PARA_STATUS, &val);
	count += snprintf(buf + count, PAGE_SIZE, "Param status:0x%02x\n", val);

	fts_i2c_read_reg_mi(client, FTS_REG_VENDOR_ID, &val);
	count += snprintf(buf + count, PAGE_SIZE, "Vendor ID:0x%02x\n", val);

	fts_i2c_read_reg_mi(client, FTS_REG_LCD_BUSY_NUM, &val);
	...
	fts_i2c_read_reg_mi(client, FTS_REG_CHARGER_MODE_EN, &val);
	count += snprintf(buf + count, PAGE_SIZE, "charge stat:0x%02x\n", val);

	fts_i2c_read_reg_mi(client, FTS_REG_INT_CNT, &val);
	count += snprintf(buf + count, PAGE_SIZE, "INT count:0x%02x\n", val);

	fts_i2c_read_reg_mi(client, FTS_REG_FLOW_WORK_CNT, &val);

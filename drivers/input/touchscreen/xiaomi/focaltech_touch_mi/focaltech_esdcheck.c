...
int idc_esdcheck_lcderror(struct fts_ts_data_mi *ts_data)
{
	int ret = 0;
	u8 val = 0;
	...
	ret = fts_i2c_read_reg_mi(client, FTS_REG_ESD_SATURATE, &val);
	if (ret < 0) {
...
*  Return:
*****************************************************************************/
static int fts_esdcheck_tp_reset(struct fts_ts_data_mi *ts_data)
{
	FTS_FUNC_ENTER();
...
*           0 - Read Chip Id pass
*****************************************************************************/
static bool get_chip_id(struct fts_ts_data_mi *ts_data)
{
	int ret = 0;
	int i = 0;
	u8 reg_value = 0;
	...
	for (i = 0; i < 3; i++) {
		...
		ret = fts_i2c_read_mi(client, &reg_addr, 1, &reg_value, 1);
		if (ret < 0) {
...
*****************************************************************************/
static bool get_flow_cnt(struct fts_ts_data_mi *ts_data)
{
	int ret = 0;
	u8 reg_value = 0;
	u8 reg_addr = 0;
...
*  Return:
*****************************************************************************/
static int esdcheck_algorithm(struct fts_ts_data_mi *ts_data)
{
	int ret = 0;
	u8 reg_value = 0;
	u8 reg_addr = 0;
...
static void esdcheck_func(struct work_struct *work)
{
	...
	struct fts_ts_data_mi *ts_data = container_of(work,
							struct fts_ts_data_mi, esdcheck_work.work);

	FTS_FUNC_ENTER();
	if (ENABLE == fts_esdcheck_data.mode) {
		if (ts_data->ic_info.is_incell) {
			fts_i2c_read_reg_mi(ts_data->client, FTS_REG_ESDCHECK_DISABLE, &val);
			if (0xA5 == val) {

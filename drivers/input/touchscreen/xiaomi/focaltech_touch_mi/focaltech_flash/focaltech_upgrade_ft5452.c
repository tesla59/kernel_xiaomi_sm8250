...
static int fts_ft5452_upgrade(struct i2c_client *client, u8 *buf, u32 len)
{
	...
	ret = fts_i2c_write_mi(client, cmd, FTS_CMD_DATA_LEN_LEN);
	if (ret < 0) {
		FTS_ERROR("data len cmd write fail");
		goto fw_reset;
	}

	ret = fts_fwupg_erase_mi(client, FTS_REASE_APP_DELAY);
	if (ret < 0) {
	...
	ecc_in_host = fts_flash_write_buf_mi(client, start_addr, buf, len, 1);
	if (ecc_in_host < 0) {
	...
	ret = fts_i2c_write_mi(client, wbuf, 7);
	if (ret < 0) {
	...
	for (i = 0; i < FTS_RETRIES_ECC_CAL; i++) {
		...
		fts_i2c_read_mi(client, wbuf, 1, reg_val, 2);
...
struct upgrade_func_mi upgrade_func_ft5452 = {
	.ctype = {0x81},
	.fwveroff = 0x010E,
	.fwcfgoff = 0x1FFB0,
	.appoff = 0x0000,

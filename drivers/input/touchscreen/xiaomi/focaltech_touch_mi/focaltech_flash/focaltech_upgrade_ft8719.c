 ...
 ***********************************************************************/
static int fts_ft8719_upgrade_mode(struct i2c_client *client, enum FW_FLASH_MODE_MI mode, u8 *buf, u32 len)
{
	int ret = 0;
	u32 start_addr = 0;
	u8 cmd[4] = { 0 };
	...
	ret = fts_fwupg_erase_mi(client, delay);
	if (ret < 0) {
	...
	/* ecc */
	ecc_in_tp = fts_fwupg_ecc_cal_mi(client, start_addr, len);
	if (ecc_in_tp < 0) {
...
static int fts_ft8719_upgrade(struct i2c_client *client, u8 *buf, u32 len)
{
	...
	if (ret < 0) {
		...
		if (fts_fwupg_reset_in_boot_mi(client) < 0) {
			FTS_ERROR("reset to normal boot fail");
		}
		return ret;
	}
...
struct upgrade_func_mi upgrade_func_ft8719 = {
	.ctype = {0x0D},
	.newmode = true,
	.fwveroff = 0x010E,
	.fwcfgoff = 0x1F80,

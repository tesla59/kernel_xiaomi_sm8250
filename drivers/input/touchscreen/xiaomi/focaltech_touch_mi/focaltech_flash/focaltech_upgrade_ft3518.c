...
static int fts_ft5452_upgrade(u8 *buf, u32 len)
{
	...
	ret = fts_fwupg_enter_into_boot_mi();
	if (ret < 0) {
	...
	ret = fts_fwupg_erase_mi(FTS_REASE_APP_DELAY);
	if (ret < 0) {
	...
	ecc_in_host = fts_flash_write_buf_mi(start_addr, buf, len, 1);
	if (ecc_in_host < 0 ) {
	...
	ret = fts_fwupg_reset_in_boot_mi();
	if (ret < 0) {

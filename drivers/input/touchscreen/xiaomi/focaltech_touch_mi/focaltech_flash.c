...
static struct upgrade_fw_mi fw_list[] = {
...
static struct upgrade_func_mi *upgrade_func_list[] = {
	&upgrade_func_ft8719,
	&upgrade_func_ft5452,
};

struct fts_upgrade_mi *fwupgrade_mi;
...
static int fts_pram_ecc_cal_algo(struct i2c_client *client, u32 start_addr, u32 ecc_length)
{
	...
	ret = fts_i2c_write_mi(client, cmd, FTS_ROMBOOT_CMD_ECC_NEW_LEN);
	if (ret < 0) {
	...
	for (i = 0; i < 100; i++) {
		msleep(1);
		ret = fts_i2c_read_mi(client, cmd, 1, val, 1);
		if (ret < 0) {
	...
	ret = fts_i2c_read_mi(client, cmd, 1, val, 2);
	if (ret < 0) {
...
static int fts_pram_ecc_cal_xor(struct i2c_client *client)
{
	...
	ret = fts_i2c_read_reg_mi(client, FTS_ROMBOOT_CMD_ECC, &reg_val);
	if (ret < 0) {
		FTS_ERROR("read pramboot ecc fail");
		return ret;
	}
...
{
	if ((NULL == fwupgrade_mi) && (NULL == fwupgrade_mi->func)) {
		FTS_ERROR("fwupgrade/func is null");
		return -EINVAL;
	}

	if (fwupgrade_mi->func->newmode) {
...
static int fts_pram_write_buf(struct i2c_client *client, u8 *buf, u32 len)
{
	...
	for (i = 0; i < packet_number; i++) {
		...
		ret = fts_i2c_write_mi(client, packet_buf, packet_len + FTS_CMD_WRITE_LEN);
		if (ret < 0) {
...
static int fts_pram_start(struct i2c_client *client)
{
	...
	ret = fts_i2c_write_mi(client, &cmd, 1);
	if (ret < 0) {
...
static int fts_pram_write_remap(struct i2c_client *client)
{
	...
	u8 *pb_buf = NULL;
	u32 pb_len = 0;
	struct fts_upgrade_mi *upg = fwupgrade_mi;
...
static int fts_pram_init(struct i2c_client *client)
{
	...
	ret = fts_i2c_read_mi(client, wbuf, 1, &reg_val, 1);
	if (ret < 0) {
	...
	wbuf[1] = reg_val;
	wbuf[2] = 0x00;
	ret = fts_i2c_write_mi(client, wbuf, 3);
	if (ret < 0) {
...
static int fts_pram_write_init(struct i2c_client *client)
{
	int ret = 0;
	bool state = 0;
	enum FW_STATUS_MI status = FTS_RUN_IN_ERROR;
	struct fts_upgrade_mi *upg = fwupgrade_mi;
	...
	ret = fts_fwupg_get_boot_state_mi(client, &status);
	if (status != FTS_RUN_IN_ROM) {
		...
		ret = fts_fwupg_reset_to_romboot_mi(client);
		if (ret < 0) {
	...
	state = fts_fwupg_check_state_mi(client, FTS_RUN_IN_PRAM);
	if (!state) {
...
***********************************************************************/
bool fts_fwupg_check_fw_valid_mi(struct i2c_client *client)
{
	int ret = 0;

	ret = fts_wait_tp_to_valid_mi(client);
	if (ret < 0) {
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_fwupg_get_boot_state_mi(struct i2c_client *client, enum FW_STATUS_MI *fw_sts)
{
	int ret = 0;
	u8 cmd[4] = { 0 };
	u32 cmd_len = 0;
	u8 val[2] = { 0 };
	struct ft_chip_mi_t ids = fts_data_mi->ic_info.ids;
	struct fts_upgrade_mi *upg = fwupgrade_mi;
	...
	ret = fts_i2c_write_mi(client, cmd, 2);
	if (ret < 0) {
	...
	if (fts_data_mi->ic_info.is_incell)
		cmd_len = FTS_CMD_READ_ID_LEN_INCELL;
	else
		cmd_len = FTS_CMD_READ_ID_LEN;
	ret = fts_i2c_read_mi(client, cmd, cmd_len, val, 2);
	if (ret < 0) {
...
***********************************************************************/
bool fts_fwupg_check_state_mi(struct i2c_client *client, enum FW_STATUS_MI rstate)
{
	int ret = 0;
	int i = 0;
	enum FW_STATUS_MI cstate = FTS_RUN_IN_ERROR;

	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		ret = fts_fwupg_get_boot_state_mi(client, &cstate);
		if (cstate == rstate)
			return true;
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_fwupg_reset_in_boot_mi(struct i2c_client *client)
{
	int ret = 0;
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_fwupg_reset_to_boot_mi(struct i2c_client *client)
{
	int ret = 0;

	FTS_INFO("send 0xAA and 0x55 to FW, reset to boot environment");

	ret = fts_i2c_write_reg_mi(client, FTS_REG_UPGRADE, FTS_UPGRADE_AA);
	if (ret < 0) {
	...
	ret = fts_i2c_write_reg_mi(client, FTS_REG_UPGRADE, FTS_UPGRADE_55);
	if (ret < 0) {
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_fwupg_reset_to_romboot_mi(struct i2c_client *client)
{
	int ret = 0;
	int i = 0;
	u8 cmd = FTS_CMD_RESET;
	enum FW_STATUS_MI state = FTS_RUN_IN_ERROR;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		ret = fts_fwupg_get_boot_state_mi(client, &state);
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_fwupg_enter_into_boot_mi(struct i2c_client *client)
{
	int ret = 0;
	bool fwvalid = false;
	bool state = false;
	...
	if (fts_data_mi->fw_forceupdate)
		fts_reset_proc_mi(10);
	else {
		fwvalid = fts_fwupg_check_fw_valid_mi(client);
		if (fwvalid) {
			ret = fts_fwupg_reset_to_boot_mi(client);
			if (ret < 0) {
	...
	if (upg->func->pramboot_supported) {
		...
		/* bootloader */
		state = fts_fwupg_check_state_mi(client, FTS_RUN_IN_BOOTLOADER);
		if (!state) {
...
static bool fts_fwupg_check_flash_status(struct i2c_client *client, u16 flash_status, int retries, int retries_delay)
{
	...
	for (i = 0; i < retries; i++) {
		...
		ret = fts_i2c_read_mi(client, &cmd, 1, val, FTS_CMD_FLASH_STATUS_LEN);
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_fwupg_erase_mi(struct i2c_client *client, u32 delay)
{
	int ret = 0;
	u8 cmd = 0;
	bool flag = false;
 ...
 ***********************************************************************/
int fts_fwupg_ecc_cal_mi(struct i2c_client *client, u32 saddr, u32 len)
{
	int ret = 0;
	u32 i = 0;
	...
	ret = fts_i2c_write_mi(client, wbuf, 1);
	if (ret < 0) {
	...
	for (i = 0; i < packet_num; i++) {
		...
		ret = fts_i2c_write_mi(client, wbuf, FTS_CMD_ECC_CAL_LEN);
		if (ret < 0) {
	...
	ret = fts_i2c_read_mi(client, wbuf, 1, val, 1);
	if (ret < 0) {
 ...
 ***********************************************************************/
int fts_flash_write_buf_mi(struct i2c_client *client, u32 saddr, u8 *buf, u32 len, u32 delay)
{
	int ret = 0;
	u32 i = 0;
	u32 j = 0;
 ...
 ***********************************************************************/
int fts_flash_read_buf_mi(struct i2c_client *client, u32 saddr, u8 *buf, u32 len)
{
	int ret = 0;
	u32 i = 0;
	u32 packet_number = 0;
	...
	for (i = 0; i < packet_number; i++) {
		...
		ret = fts_i2c_write_mi(client, wbuf, FTS_CMD_READ_LEN);
		if (ret < 0) {
		...
		ret = fts_i2c_read_mi(client, NULL, 0, buf + offset, packet_len);
		if (ret < 0) {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_flash_read_mi(struct i2c_client *client, u32 addr, u8 *buf, u32 len)
{
	int ret = 0;
	...
	ret = fts_fwupg_enter_into_boot_mi(client);
	if (ret < 0) {
		FTS_ERROR("enter into pramboot/bootloader fail");
		goto read_flash_err;
	}

	ret = fts_flash_read_buf_mi(client, addr, buf, len);
	if (ret < 0) {
...
read_flash_err:
	...
	ret = fts_fwupg_reset_in_boot_mi(client);
	if (ret < 0) {
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_upgrade_bin_mi(struct i2c_client *client, char *fw_name, bool force)
{
	int ret = 0;
	u32 fw_file_len = 0;
	u8 *fw_file_buf = NULL;
	...
	if (ret < 0) {
		...
		fts_fwupg_reset_in_boot_mi(client);
		goto err_bin;
	}
...
static int fts_param_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
	...
	ret = fts_i2c_read_reg_mi(client, FTS_REG_IDE_PARA_VER_ID, ver);
	if (ret < 0) {
...
static bool fts_param_need_upgrade(struct i2c_client *client)
{
	...
	ret = fts_i2c_read_reg_mi(client, FTS_REG_IDE_PARA_STATUS, &val);
	if (ret < 0) {
...
static int fts_fwupg_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
	...
	ret = fts_i2c_read_reg_mi(client, FTS_REG_FW_VER, ver);
	if (ret < 0) {
...
static bool fts_fwupg_need_upgrade(struct i2c_client *client)
{
	...
	u8 fw_ver_in_host = 0;
	u8 fw_ver_in_tp = 0;
	struct fts_ts_data_mi *ts_data = NULL;
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_fwupg_upgrade_mi(struct i2c_client *client, struct fts_upgrade_mi *upg)
{
	int ret = 0;
 ...
 ***********************************************************************/
void fts_fwupg_auto_upgrade(struct fts_ts_data_mi *ts_data)
{
	int ret = 0;
	...
	ret = fts_fwupg_upgrade_mi(client, upg);
	if (ret < 0)
 ...
 * return 0 if success, otherwise return error code
 */
int fts_fwupg_get_vendorid(struct fts_ts_data_mi *ts_data, u16 *vid)
{
	int ret = 0;
	bool fwvalid = false;
	u8 vendor_id = 0;
	...
	fwvalid = fts_fwupg_check_fw_valid_mi(client);
	if (fwvalid) {
		ret = fts_i2c_read_reg_mi(client, FTS_REG_VENDOR_ID, &vendor_id);
		if (ts_data->ic_info.is_incell)
			ret = fts_i2c_read_reg_mi(client, FTS_REG_MODULE_ID, &module_id);
	} else {
		fwcfg_addr = upg->func->fwcfgoff;
		ret = fts_flash_read_mi(client, fwcfg_addr, cfgbuf, FTS_HEADER_LEN);
 ...
 */
static int fts_fwupg_get_fw_file(struct fts_ts_data_mi *ts_data)
{
	struct upgrade_fw_mi *fw = &fw_list[0];
	struct fts_upgrade_mi *upg = fwupgrade_mi;
...
{
	int ret = 0;
	struct fts_ts_data_mi *ts_data = container_of(work,
							struct fts_ts_data_mi, fwupg_work);
...
#endif
	if (FTS_FOD)
		fts_fod_recovery_mi(ts_data->client);
	fts_irq_enable_mi();
	ts_data->fw_loading = 0;
}

static int fts_get_lockdown_info(struct fts_ts_data_mi *ts_data)
{
	int i, ret = -EINVAL;
	char buf[128];
	...
	ret = fts_flash_read_mi(ts_data->client, ts_data->pdata->lockdown_info_addr, ts_data->lockdown_info, FTS_LOCKDOWN_INFO_SIZE);
	if (ret < 0) {
 ...
 *  Return: return 0 if success, otherwise return error code
 *****************************************************************************/
int fts_fwupg_init_mi(struct fts_ts_data_mi *ts_data)
{
	int i = 0;
	int j = 0;
	int ret;
	int ic_stype = ts_data->ic_info.ids.type;
	struct upgrade_func_mi *func = upgrade_func_list[0];
	...
	fwupgrade_mi = (struct fts_upgrade_mi *)kzalloc(sizeof(*fwupgrade_mi), GFP_KERNEL);
	if (NULL == fwupgrade_mi) {
		FTS_ERROR("malloc memory for upgrade fail");
		return -ENOMEM;
	}

	if (1 == func_count) {
		fwupgrade_mi->func = func;
	} else {
 ...
 *  Return:
 *****************************************************************************/
int fts_fwupg_exit_mi(struct fts_ts_data_mi *ts_data)
{
	FTS_FUNC_ENTER();
	if (fwupgrade_mi) {
		kfree(fwupgrade_mi);

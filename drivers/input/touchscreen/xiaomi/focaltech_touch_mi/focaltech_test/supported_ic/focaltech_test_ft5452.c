...
static int start_scan(void)
{
	...
	ret = read_reg_mi(DEVIDE_MODE_ADDR, &val);
	...
	val |= 0x80;
	ret = write_reg_mi(DEVIDE_MODE_ADDR, val);
...
static int read_rawdata(u8 freq, u8 line_num, int byte_num, int *rev_buffer)
{
	...
	ret = write_reg_mi(REG_LINE_NUM, line_num);/* set row addr */

	if (ret) {
		FTS_TEST_SAVE_ERR("failed to write REG_LINE_NUM!");
	}

	read_data = fts_malloc_dma_mi(byte_num * sizeof(u8));
	...
	i2c_w_buffer = fts_malloc_dma_mi(3);
	...
	if (!ret) {
		ret = fts_i2c_read_write_mi(i2c_w_buffer, 1, read_data, bytes_per_time);
	}
	...
	for (i = 1; i < read_num; i++) {
		...
		if (i == read_num - 1) { /* last packet &*/
			ret = fts_i2c_read_write_mi(NULL, 0, read_data + BYTES_PER_TIME * i, byte_num - BYTES_PER_TIME * i);
		} else {
			ret = fts_i2c_read_write_mi(NULL, 0, read_data + BYTES_PER_TIME * i, BYTES_PER_TIME);
		}
	}
...
static int get_tx_sc_cb(u8 index, u8 *pcb_value)
{
	...
	if (index < 128) { /* single read */
		*pcb_value = 0;
		write_reg_mi(REG_SC_CB_ADDRR, index);
		ret = read_reg_mi(REG_SC_CB_BUF0, pcb_value);
	} else { /* sequential read length index-128 */
		write_reg_mi(REG_SC_CB_ADDRR, 0);
		*w_buffer = REG_SC_CB_BUF0;
		ret = fts_i2c_read_write_mi(w_buffer, 1, pcb_value, index - 128);
		/*
...
static int get_channel_num_no_mapping(void)
{
	...
	ret = read_reg_mi(REG_TX_NOMAPPING_NUM,  buffer);
	...
	ret = read_reg_mi(REG_RX_NOMAPPING_NUM,  buffer);
...
static int mapping_switch(u8 str_switch)
{
	...
	u8 ch_pattern = 0xFF;
	u8 reg_data = 0xFF;
	ret = read_reg_mi(REG_PATTERN_5452, &ch_pattern);
	...
	if (1 == ch_pattern) { /* 1: V3 Pattern */
		reg_data = 0xFF;
		ret = read_reg_mi(REG_MAPPING_SWITCH, &reg_data);
		...
		if (str_switch != reg_data) {
			ret = write_reg_mi(REG_MAPPING_SWITCH, str_switch);  /* 0-mapping 1-no mampping */
...
static int weak_short_get_adc_data(int all_adc_data_len, int *rev_buffer, u8 cmd)
{
	...
	bool adc_ok = false;
	FTS_TEST_FUNC_ENTER();
	data_send = fts_malloc_dma_mi(read_data_len + 1);
	...
	ret = write_reg_mi(0x07, cmd);/* test weak short once, after host send an enable command */
	...
	for (i = 0; i < 100 * 5; i++) {
		sys_delay_mi(10);
		ret = read_reg_mi(0x07, &data); /* data ready,and FW set reg 0x07 to 0. */
	...
	sys_delay_mi(300);
	*data_send = 0xF4;
	ret = fts_i2c_read_write_mi(data_send, 1, data_send + 1, read_data_len);
...
/*  */
static bool short_test_get_channel_num_mi(int *fm_short_resistance, int *adc_data, int offset, bool *is_weak_short_mut)
{
	int ret = 0;
	int i;
	int count = 0;
...
static bool short_test_channel_to_gnd(int *fm_short_resistance, int *adc_data, int offset, bool *is_weak_short_gnd)
{
	...
	error_ch = fts_malloc_mi(total_num * sizeof(u8));
	...
	min_70k_ch = fts_malloc_mi(total_num * sizeof(u8));
	...
	w_buf = fts_malloc_dma_mi((total_num + 3) * sizeof(u8));
	...
	tmp_adc_data = fts_malloc_dma_mi(total_num * total_num * sizeof(int));
	...
	fg_short_resistance =  fts_malloc_dma_mi(total_num * sizeof(int));
	...
	if (error_num > 0) {
		...
		*(w_buf + 1) = (u8)error_num;
		...
		ret = fts_i2c_read_write_mi(w_buf, (u16)(error_num + 2), NULL, 0);
	...
	if (min_70k_num > 0) {
		ret = write_reg_mi(SHORT_TEST_RES_LEVEL, 0x00);
		...
		ret = fts_i2c_read_write_mi(w_buf, (u16)(min_70k_num + 2), NULL, 0);
...
static bool short_test_channel_to_channel(int *fm_short_resistance, int *adc_data, int offset)
{
	...
	all_adc_data_num = total_num + 1;
	min_cc = ft5452_basic_thr.weak_short_test_cc;
	f_origin_resistance =  fts_malloc_mi(total_num * sizeof(int));
	...
	ret = write_reg_mi(SHORT_TEST_RES_LEVEL, 0x01);

	if (ret) {
		goto TEST_END;
	}

	w_buf = fts_malloc_mi(total_num * sizeof(u8));
	if (NULL == w_buf) {
		FTS_TEST_SAVE_ERR("w_buf buffer malloc fail");
		return -ENOMEM;
	}
...
static int ft5452_enter_factory_mode_mi(void)
{
	int ret = 0;
	int i = 0;
	FTS_TEST_FUNC_ENTER();
...
static int ft5452_rawdata_test(bool *test_result)
{
	...
	ret = read_reg_mi(REG_NORMALIZE_TYPE, &origin_value);
	...
	ret =  read_reg_mi(REG_FRE_LIST, &fre);
	...
	ret =  read_reg_mi(REG_FIR_ENABLE, &fir);
	...
	if (origin_value != 0x01) {
		ret = write_reg_mi(REG_NORMALIZE_TYPE, 0x01);
	...
	/* set frequecy high */
	ret = write_reg_mi(REG_FRE_LIST, 0x81);
	...
	ret = write_reg_mi(REG_FIR_ENABLE, 1);
	...
	ret = write_reg_mi(REG_NORMALIZE_TYPE, origin_value);
	...
	ret = write_reg_mi(REG_FRE_LIST, fre);
	...
	ret = write_reg_mi(REG_FIR_ENABLE, fir);
...
static int ft5452_uniformity_test(bool *test_result)
{
	...
	rawdata = test_data.buffer;
	rx_linearity = fts_malloc_mi(buf_len);
	...
	tx_linearity = fts_malloc_mi(buf_len);
	...
	ret = read_reg_mi(REG_FIR_ENABLE, &fir_value);
	...
	ret = write_reg_mi(REG_FIR_ENABLE, fir_value);
...
static int ft5452_scap_cb_test(bool *test_result)
{
	...
	ret = read_reg_mi(REG_WATER_CHANNEL_SELECT, &wc_value);
	...
	ret = read_reg_mi(REG_SC_WORK_MODE, &sc_wrok_mode);
	...
	temp_cb = fts_malloc_dma_mi(((tx_num + 1) * rx_num) * sizeof(u8));
	...
	for (i = 0; i < 3; i++) {
		/* waterproof CB */
		ret = write_reg_mi(REG_SC_WORK_MODE, 1);/* ScWorkMode:  1:waterproof 0:Non-waterproof */
		...
		ret = write_reg_mi(REG_SC_CB_ADDRR, 0);
		...
		*/
		...
		ret = write_reg_mi(REG_SC_WORK_MODE, 0);/* ScWorkMode:  1:waterproof 0:Non-waterproof */
	...
	ret = write_reg_mi(REG_SC_WORK_MODE, sc_wrok_mode);/* set the origin value */
...
static int ft5452_panel_differ_test(bool *test_result)
{
	...
	abs_differ_data = fts_malloc_mi(((tx_num + 1) * rx_num) * sizeof(int));
	...
	scap_rawdata = fts_malloc_dma_mi(((tx_num + 1) * rx_num) * sizeof(int));
	...
	ret = read_reg_mi(REG_NORMALIZE_TYPE, &origin_rawdata_type);/* read the original value */
	...
	if (origin_rawdata_type != 0) {
		ret = write_reg_mi(REG_NORMALIZE_TYPE, 0x00);
		sys_delay_mi(50);
	...
	ret = read_reg_mi(REG_FRE_LIST, &origin_frequecy); /* read the original value */
	...
	ret = read_reg_mi(REG_FIR_ENABLE, &origin_fir_state);/* read the original value */
	...
	ret = write_reg_mi(REG_FIR_ENABLE, 0);
	sys_delay_mi(50);
	...
	ret = write_reg_mi(REG_NORMALIZE_TYPE, origin_rawdata_type);/* set to original value */
	ret = write_reg_mi(REG_FRE_LIST, origin_frequecy); /* set to original value */
	ret = write_reg_mi(REG_FIR_ENABLE, origin_fir_state); /* set to original value */
...
static int ft5452_short_test(bool *test_result, bool enable)
{
	...
	sys_delay_mi(200);
	adc_data = fts_malloc_dma_mi((all_adc_data_num + 1) * sizeof(int));
	...
	fm_short_resistance = fts_malloc_dma_mi(total_num * sizeof(int));
	...
	ret = read_reg_mi(SHORT_TEST_RES_LEVEL, &stall_value);
	ret = write_reg_mi(SHORT_TEST_RES_LEVEL, 0x01);
	...
	tmp_result = short_test_get_channel_num_mi(fm_short_resistance, adc_data, offset, &is_weak_short_mut);
...
TEST_END:
	...
	ret = write_reg_mi(SHORT_TEST_RES_LEVEL, stall_value);
	if (enable) {
...
static bool start_test_ft5452(void)
{
	...
	for (item_count = 0; item_count < test_data.test_num; item_count++) {
		...
		if (CODE_FT5452_ENTER_FACTORY_MODE == item_code) {
			ret = ft5452_enter_factory_mode_mi();
...
static void init_testitem_ft5452(char *ini)
{
	...
	FTS_TEST_FUNC_ENTER();
	/* RawData Test */
	GetPrivateProfileString_mi("TestItem", "RAWDATA_TEST", "1", str, ini);
	ft5452_item.rawdata_test = fts_atoi_mi(str);
	/* uniformity_test */
	GetPrivateProfileString_mi("TestItem", "UNIFORMITY_TEST", "0", str, ini);
	ft5452_item.uniformity_test = fts_atoi_mi(str);
	/* scap_cb_test */
	GetPrivateProfileString_mi("TestItem", "SCAP_CB_TEST", "1", str, ini);
	ft5452_item.scap_cb_test = fts_atoi_mi(str);
	/* scap_rawdata_test */
	GetPrivateProfileString_mi("TestItem", "SCAP_RAWDATA_TEST", "1", str, ini);
	ft5452_item.scap_rawdata_test = fts_atoi_mi(str);
	/* panel differ_TEST */
	GetPrivateProfileString_mi("TestItem", "PANEL_DIFFER_TEST", "1", str, ini);
	ft5452_item.panel_differ_test = fts_atoi_mi(str);
	/* weak_short_test */
	GetPrivateProfileString_mi("TestItem", "WEAK_SHORT_CIRCUIT_TEST", "1", str, ini);
	ft5452_item.weak_short_test = fts_atoi_mi(str);
	FTS_TEST_FUNC_EXIT();
}
...
static int ft5452_open_selftest(void)
{
	...
	if (fts_data_mi) {
		ret = ft5452_get_rawdata(fts_data_mi->client, 0, rawdata, test_data.screen_param.tx_num * test_data.screen_param.rx_num * 2);
		if (ret) {
		...
		for (i = 0; i < (test_data.screen_param.tx_num * test_data.screen_param.rx_num); i++) {
			...
			if (rawdata[i] < fts_data_mi->pdata->open_min) {
				ret = SELFTEST_FAIL;
				goto out;
			}
		}
...
static void init_basicthreshold_ft5452(char *ini)
{
	...
	FTS_TEST_FUNC_ENTER();
	/* Uniformity */
	GetPrivateProfileString_mi("Basic_Threshold", "UniformityTest_Check_Tx", "0", str, ini);
	ft5452_basic_thr.uniformity_check_tx = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "UniformityTest_Check_Rx", "0", str, ini);
	ft5452_basic_thr.uniformity_check_rx = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "UniformityTest_Check_MinMax", "0", str, ini);
	ft5452_basic_thr.uniformity_check_min_max = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "UniformityTest_Tx_Hole", "20", str, ini);
	ft5452_basic_thr.uniformity_tx_hole = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "UniformityTest_Rx_Hole", "20", str, ini);
	ft5452_basic_thr.uniformity_rx_hole = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "UniformityTest_MinMax_Hole", "70", str, ini);
	ft5452_basic_thr.uniformity_min_max_hole = fts_atoi_mi(str);
	/* scap cb */
	GetPrivateProfileString_mi("Basic_Threshold", "SCapCbTest_OFF_Min", "0", str, ini);
	ft5452_basic_thr.scap_cb_test_off_min = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "SCapCbTest_OFF_Max", "240", str, ini);
	ft5452_basic_thr.scap_cb_test_off_max = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "SCapCbTest_ON_Min", "0", str, ini);
	ft5452_basic_thr.scap_cb_test_on_min = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "SCapCbTest_ON_Max", "240", str, ini);
	ft5452_basic_thr.scap_cb_test_on_max = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "ScapCBTest_SetWaterproof_OFF", "0", str, ini);
	ft5452_basic_thr.scap_cb_test_set_waterproof_off = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "ScapCBTest_SetWaterproof_ON", "240", str, ini);
	ft5452_basic_thr.scap_cb_test_set_waterproof_on = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "SCapCBTest_LetTx_Disable", "0", str, ini);
	ft5452_basic_thr.scap_cb_test_let_tx_disable = fts_atoi_mi(str);
	/* scap rawdata */
	GetPrivateProfileString_mi("Basic_Threshold", "SCapRawDataTest_OFF_Min", "5000", str, ini);
	ft5452_basic_thr.scap_rawdata_test_off_min = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "SCapRawDataTest_OFF_Max", "8500", str, ini);
	ft5452_basic_thr.scap_rawdata_test_off_max = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "SCapRawDataTest_ON_Min", "5000", str, ini);
	ft5452_basic_thr.scap_rawdata_test_on_min = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "SCapRawDataTest_ON_Max", "8500", str, ini);
	ft5452_basic_thr.scap_rawdata_test_on_max = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "SCapRawDataTest_SetWaterproof_OFF", "1", str, ini);
	ft5452_basic_thr.scap_rawdata_test_set_waterproof_off = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "SCapRawDataTest_SetWaterproof_ON", "0", str, ini);
	ft5452_basic_thr.scap_rawdata_test_set_waterproof_on = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "SCapRawDataTest_LetTx_Disable", "0", str, ini);
	ft5452_basic_thr.scap_rawdata_test_let_tx_disable = fts_atoi_mi(str);
	/* panel differ */
	GetPrivateProfileString_mi("Basic_Threshold", "PanelDifferTest_Min", "150", str, ini);
	ft5452_basic_thr.panel_differ_test_min = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "PanelDifferTest_Max", "1000", str, ini);
	ft5452_basic_thr.panel_differ_test_max = fts_atoi_mi(str);
	/* weak short */
	GetPrivateProfileString_mi("Basic_Threshold", "WeakShortTest_CG", "2000", str, ini);
	ft5452_basic_thr.weak_short_test_cg = fts_atoi_mi(str);
	GetPrivateProfileString_mi("Basic_Threshold", "WeakShortTest_CC", "2000", str, ini);
	ft5452_basic_thr.weak_short_test_cc = fts_atoi_mi(str);
	FTS_TEST_FUNC_EXIT();
}
...
{
	FTS_TEST_FUNC_ENTER();
	OnInit_InvalidNode_mi(ini);
	OnInit_DThreshold_RawDataTest_mi(ini);
	OnInit_DThreshold_SCapRawDataTest_mi(ini);
	OnInit_DThreshold_SCapCbTest_mi(ini);
	OnInit_DThreshold_RxLinearityTest_mi(ini);
	OnInit_DThreshold_TxLinearityTest_mi(ini);
	OnInit_DThreshold_PanelDifferTest_mi(ini);
	FTS_TEST_FUNC_EXIT();
}
...
static void set_testitem_sequence_ft5452(void)
{
	...
	FTS_TEST_FUNC_ENTER();
	/* Enter Factory Mode */
	fts_set_testitem_mi(CODE_FT5452_ENTER_FACTORY_MODE);

	/* RawData Test */
	if (ft5452_item.rawdata_test == 1) {
		fts_set_testitem_mi(CODE_FT5452_RAWDATA_TEST);
	}

	/* Rawdata Uniformity Test */
	if (ft5452_item.uniformity_test == 1) {
		fts_set_testitem_mi(CODE_FT5452_UNIFORMITY_TEST);
	}

	/* scap_cb_test */
	if (ft5452_item.scap_cb_test == 1) {
		fts_set_testitem_mi(CODE_FT5452_SCAP_CB_TEST);
	}

	/* scap_rawdata_test */
	if (ft5452_item.scap_rawdata_test == 1) {
		fts_set_testitem_mi(CODE_FT5452_SCAP_RAWDATA_TEST);
	}

	/* panel differ_TEST */
	if (ft5452_item.panel_differ_test == 1) {
		fts_set_testitem_mi(CODE_FT5452_PANELDIFFER_TEST);
	}

	/* weak_short_test */
	if (ft5452_item.weak_short_test == 1) {
		fts_set_testitem_mi(CODE_FT5452_WEAK_SHORT_CIRCUIT_TEST);
	}
...
static int ft5452_startscan(struct i2c_client *client)
{
	...
	ret = read_reg_mi(DEVIDE_MODE_ADDR, &OldMode);
	...
	ReCode = fts_i2c_write_reg_mi(client, DEVIDE_MODE_ADDR, 0xC0);
	if (ReCode >= 0)
	{
		while (times++ < MaxTimes)
		{
			msleep(8);
			ReCode = fts_i2c_read_reg_mi(client, DEVIDE_MODE_ADDR, &RegVal);
			if (RegVal == 0x40)
				break;
		}
		if (times > MaxTimes)
...
static int ft5452_get_rawdata(struct i2c_client *client, int is_diff, int *rawdata, int len)
{
	...
	fts_i2c_write_reg_mi(client, FACTORY_REG_DATA_SELECT, is_diff);
	/*
	if (ft5452_startscan(client) < 0)
		return -EFAULT;
		*/
	start_scan();
	fts_i2c_write_reg_mi(client, FACTORY_REG_LINE_ADDR, 0xAA);
	if (len <= 256)
		fts_i2c_read_mi(client, &reg, 1, regdata, len);
	else {
		fts_i2c_read_mi(client, &reg, 1, regdata, 256);
		remain_bytes = len - 256;
		for (i = 1; remain_bytes > 0; i++) {
			if (remain_bytes > 256)
			  fts_i2c_read_mi(client, &reg, 0, regdata + i * 256, 256);
			else
				fts_i2c_read_mi(client, &reg, 0, regdata + i * 256, remain_bytes);
			remain_bytes -= 256;
		}
	}
	for (i = 0; i < len;) {
	...
	fts_i2c_write_reg_mi(client, FACTORY_REG_DATA_SELECT, 0);
	return 0;
}
...
static int ft5452_data_dump(int *rawdata, int *differ_data)
{
	...
	if (fts_data_mi) {
		ret = ft5452_get_rawdata(fts_data_mi->client, 1, differ_data, test_data.screen_param.tx_num * test_data.screen_param.rx_num * 2);
		if (ret) {
...
struct test_funcs_mi test_func_ft5452 = {

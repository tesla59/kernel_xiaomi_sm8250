...
Note:
*************************************************************/
char *ini_str_trim_r_mi(char *buf)
{
	int len, i;
...
Note:
*************************************************************/
char *ini_str_trim_l_mi(char *buf)
{
	int len, i;
	char tmp[MAX_CFG_BUF_LINE_LEN + 1] = { 0 };
...
int ini_get_key_data(char *filedata)
{
	...
	while (1) { /*find section */
		...
		dataoff += n;
		n = strlen(ini_str_trim_l_mi(ini_str_trim_r_mi(buf1)));
...
int GetPrivateProfileString_mi(char *section, char *ItemName, char *defaultvalue, char *returnValue, char *IniFile)
{
...
int fts_test_get_ini_size_mi(char *config_name)
{
...
int fts_test_read_ini_data_mi(char *config_name, char *config_buf)
{
	const struct firmware *fw = NULL;
	struct device *dev = NULL;
	int ret = 0;
...
int init_interface(char *ini)
{
	...
	/* IC type */
	GetPrivateProfileString_mi("Interface", "IC_Type", "FT5X36", str, ini);
	ic_type = fts_ic_table_get_ic_code_from_ic_name_mi(str);
	if (0 == ic_type) {
	...
	/* Normalize Type */
	GetPrivateProfileString_mi("Interface", "Normalize_Type", 0, str, ini);
	test_data.screen_param.normalize = fts_atoi_mi(str);
...
int set_param_data(char *test_param)
{
	...
	test_data.ini_data = (struct _ini_data_mi *)fts_malloc_mi(sizeof(struct _ini_data_mi) * MAX_KEYWORD_NUM);
	...
	/* Get test function */
	ret = init_test_funcs_mi(test_data.screen_param.selected_ic);
	if (ret < 0) {
 ...
 */
int fts_test_get_testparam_from_ini_mi(char *config_name)
{
	int ret = 0;
	...
	inisize = fts_test_get_ini_size_mi(config_name);
	...
	ini_file_data = fts_malloc_mi(inisize + 1);
	...
	ret = fts_test_read_ini_data_mi(config_name, ini_file_data);
	if (ret) {

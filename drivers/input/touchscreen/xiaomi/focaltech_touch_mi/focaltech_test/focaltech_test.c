...
*****************************************************************************/
struct fts_test_data_mi test_data;

struct test_funcs_mi *test_funcs_list[] = {
	&test_func_ft5452,
	&test_func_ft8719,
};

static struct test_ic_type_mi ic_types[] = {
	{"FT5X22", 6, IC_FT5X46},
	{"FT5X46", 6, IC_FT5X46},
...
* functions body
*****************************************************************************/
void sys_delay_mi(int ms)
{
	msleep(ms);
}

int focal_abs_mi(int value)
{
	if (value < 0)
		value = 0 - value;
...
void *fts_malloc_dma_mi(size_t size)
{
	return kzalloc(size, GFP_DMA);
}

void *fts_malloc_mi(size_t size)
{
	return kzalloc(size, GFP_KERNEL);
}

void fts_free_proc_mi(void *p)
{
	kfree(p);
}
 ...
 *******************************************************************/
int fts_test_i2c_read_mi(u8 *writebuf, int writelen, u8 *readbuf, int readlen)
{
	int ret = 0;
	if (NULL == fts_data_mi) {
	...
	ret = fts_i2c_read_mi(fts_data_mi->client, writebuf, writelen, readbuf, readlen);
...
int fts_test_i2c_write_mi(u8 *writebuf, int writelen)
{
	int ret = 0;
	if (NULL == fts_data_mi) {
		FTS_TEST_ERROR("fts_data is null, no test");
	...
	ret = fts_i2c_write_mi(fts_data_mi->client, writebuf, writelen);
	if (ret < 0)
		return ret;
	else
		return 0;
...
int read_reg_mi(u8 addr, u8 *val)
{
	return fts_test_i2c_read_mi(&addr, 1, val, 1);
}

int write_reg_mi(u8 addr, u8 val)
{
	int ret;
	u8 cmd[2] = {0};
	...
	cmd[0] = addr;
	cmd[1] = val;
	ret = fts_test_i2c_write_mi(cmd, 2);
...
int enter_work_mode_mi(void)
{
	...
	ret = read_reg_mi(DEVIDE_MODE_ADDR, &mode);
	if ((0 == ret) && (((mode >> 4) & 0x07) == 0x00))
		return 0;

	for (i = 0; i < ENTER_WORK_FACTORY_RETRIES; i++) {
		ret = write_reg_mi(DEVIDE_MODE_ADDR, 0x00);
		if (0 == ret) {
			for (j = 0; j < 20; j++) {
				ret = read_reg_mi(DEVIDE_MODE_ADDR, &mode);
				if ((0 == ret) && (((mode >> 4) & 0x07) == 0x00))
...
int enter_factory_mode_mi(void)
{
	int ret = 0;
	u8 mode = 0;
	int i = 0;
	...
	for (i = 0; i < ENTER_WORK_FACTORY_RETRIES; i++) {
		ret = write_reg_mi(DEVIDE_MODE_ADDR, 0x40);
		if (0 == ret) {
			for (j = 0; j < 20; j++) {
				ret = read_reg_mi(DEVIDE_MODE_ADDR, &mode);
				if ((0 == ret) && (((mode >> 4) & 0x07) == 0x04)) {
...
***********************************************************************/
u8 fts_i2c_read_write_mi(unsigned char *writebuf, int  writelen, unsigned char *readbuf, int readlen)
{
	int ret;

	if (readlen > 0) {
		ret = fts_test_i2c_read_mi(writebuf, writelen, readbuf, readlen);
	} else {
		ret = fts_test_i2c_write_mi(writebuf, writelen);
	}
 ...
 */
int read_mass_data_mi(u8 addr, int byte_num, int *buf)
{
	int ret = 0;
	int i = 0;
	u8 reg_addr = 0;
	...
	data = (u8 *)fts_malloc_dma_mi(byte_num * sizeof(u8));
	if (NULL == data) {
	...
	reg_addr = addr;
	ret = fts_test_i2c_read_mi(&reg_addr, 1, data, read_num);
	if (ret) {
	...
	for (i = 1; i < packet_num; i++) {
		...
		ret = fts_test_i2c_read_mi(NULL, 0, data + offset, read_num);
		if (ret) {
...
int chip_clb_incell_mi(void)
{
	...
	/* start clb */
	ret = write_reg_mi(FACTORY_REG_CLB, 0x04);
	if (ret) {
	...
	while (times++ < FACTORY_TEST_RETRY) {
		...
		ret = read_reg_mi(FACTORY_REG_CLB, &val);
		if ((0 == ret) && (0x02 == val)) {
			/* clb ok */
			break;
		} else
...
int wait_state_update_mi(void)
{
	...
	while (times++ < FACTORY_TEST_RETRY) {
		...
		state = 0xFF;
		ret = read_reg_mi(FACTORY_REG_PARAM_UPDATE_STATE, &state);
		if ((0 == ret) && (0x00 == state))
			break;
		else
 ...
 */
int get_rawdata_incell_mi(int *data)
{
	int ret = 0;
	u8 tx_num = 0;
	u8 rx_num = 0;
	...
	ret = read_mass_data_mi(FACTORY_REG_RAWDATA_ADDR, va_num * 2, data);
	if (ret) {
	...
	ret = write_reg_mi(FACTORY_REG_LINE_ADDR, 0xAE);
	if (ret) {
		FTS_TEST_SAVE_ERR("wirte AE to reg01 fail\n");
		return ret;
	}
	ret = read_mass_data_mi(FACTORY_REG_RAWDATA_ADDR, key_num * 2, data + va_num);
	if (ret) {
 ...
 */
int get_cb_incell_mi(u16 saddr, int byte_num, int *cb_buf)
{
	int ret = 0;
	int i = 0;
	u8 cb_addr = 0;
	...
	for (i = 0; i < packet_num; i++) {
		...
		ret = write_reg_mi(FACTORY_REG_CB_ADDR_H, addr_h);
		if (ret) {
			FTS_TEST_SAVE_ERR("write cb addr high fail\n");
			goto TEST_CB_ERR;
		}
		ret = write_reg_mi(FACTORY_REG_CB_ADDR_L, addr_l);
		if (ret) {
			FTS_TEST_SAVE_ERR("write cb addr low fail\n");
			goto TEST_CB_ERR;
		}

		ret = fts_test_i2c_read_mi(&cb_addr, 1, data + offset, read_num);
		if (ret) {
 ...
 */
int weakshort_get_adc_data_incell_mi(u8 retval, u8 ch_num, int byte_num, int *adc_buf)
{
	int ret = 0;
	int times = 0;
	u8 short_state = 0;
	...
	/* Start ADC sample */
	ret = write_reg_mi(FACTORY_REG_SHORT_TEST_EN, 0x01);
	if (ret) {
		FTS_TEST_SAVE_ERR("start short test fail\n");
		goto ADC_FAIL;
	}
	sys_delay_mi(ch_num * FACTORY_TEST_DELAY);

	for (times = 0; times < FACTORY_TEST_RETRY; times++) {
		ret = read_reg_mi(FACTORY_REG_SHORT_TEST_STATE, &short_state);
	...
	ret = read_mass_data_mi(FACTORY_REG_SHORT_ADDR, byte_num, adc_buf);
	if (ret) {
 ...
 */
void show_data_incell_mi(int *data, bool include_key)
{
	int row = 0;
	int col = 0;
 ...
 */
bool compare_data_incell_mi(int *data, int min, int max, int vk_min, int vk_max, bool include_key)
{
	int row = 0;
	int col = 0;
	int value = 0;
...
* Return: none.
***********************************************************************/
bool compare_detailthreshold_data_incell_mi(int *data, int *data_min, int *data_max, bool include_key)
{
	int row, col;
	int value;
	bool tmp_result = true;
 ...
 */
void save_testdata_incell_mi(int *data, char *test_num, int index, u8 row, u8 col, u8 item_count)
{
	int len = 0;
	int i = 0, j = 0;
 ...
 */
u32 fts_ic_table_get_ic_code_from_ic_name_mi(char *ic_name)
{
	int i = 0;
	int type_size = 0;
 ...
 */
int init_test_funcs_mi(u32 ic_type)
{
	int i = 0;
	struct test_funcs_mi *funcs = test_funcs_list[0];
...
* Return:
**********************************************************************/
void fts_set_testitem_mi(u8 itemcode)
{
...
{
	test_data.testresult = fts_malloc_mi(BUFF_LEN_TESTRESULT_BUFFER);
	if (NULL == test_data.testresult)
		goto ERR;

	test_data.store_all_data = fts_malloc_mi(FTS_TEST_STORE_DATA_SIZE);
	if (NULL == test_data.store_all_data)
		goto ERR;

	test_data.store_msg_area = fts_malloc_mi(BUFF_LEN_STORE_MSG_AREA);
	if (NULL == test_data.store_msg_area)
		goto ERR;

	test_data.msg_area_line2 = fts_malloc_mi(BUFF_LEN_MSG_AREA_LINE2);
	if (NULL == test_data.msg_area_line2)
		goto ERR;

	test_data.store_data_area = fts_malloc_mi(BUFF_LEN_STORE_DATA_AREA);
	if (NULL == test_data.store_data_area)
		goto ERR;

	test_data.tmp_buffer = fts_malloc_mi(BUFF_LEN_TMP_BUFFER);
...
int get_tx_rx_num(u8 tx_rx_reg, u8 *ch_num, u8 ch_num_max)
{
	...
	for (i = 0; i < 3; i++) {
		ret = read_reg_mi(tx_rx_reg, ch_num);
...
static int fts_test_init_basicinfo(void)
{
	...
	ret = read_reg_mi(REG_FW_VERSION, &val);
	FTS_TEST_SAVE_INFO("FW version:0x%02x\n", val);

	ret = read_reg_mi(REG_VA_TOUCH_THR, &val);
	test_data.va_touch_thr = val;
	ret = read_reg_mi(REG_VKEY_TOUCH_THR, &val);
...
static int fts_test_main_init(bool enable)
{
	...
	ret = malloc_struct_DetailThreshold_mi();
	if (ret < 0) {
	...
	test_data.buffer = (int *)fts_malloc_dma_mi(len * sizeof(int));
...
int fts_test_get_testparams(char *config_name)
{
	...
	ret = fts_test_get_testparam_from_ini_mi(config_name);
...
static int fts_test_entry(char *ini_file_name)
{
	...
	if (true == fts_test_start()) {
		fts_data_mi->factory_test_result = true;
		FTS_TEST_SAVE_INFO("\n=======Tp test pass. \n\n");
	} else {
		fts_data_mi->factory_test_result = false;
		FTS_TEST_SAVE_INFO("\n=======Tp test failure. \n\n");
	}
...
{
	free_struct_DetailThreshold_mi();
	if (enable)
...
static ssize_t fts_test_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!fts_data_mi)
		return -EINVAL;
	if (fts_data_mi->factory_test_result)
...
{
	char fwname[128] = {0};
	struct fts_ts_data_mi *ts_data = fts_data_mi;
	...
	fts_esdcheck_switch_mi(DISABLE);
#endif

	fts_test_entry(fwname);

#if defined(FTS_ESDCHECK_EN) && (FTS_ESDCHECK_EN)
	fts_esdcheck_switch_mi(ENABLE);
#endif
...
static int fts_i2c_test(void)
{
	...
	do {
		ret = fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_CHIP_ID, &chip_id[0]);
		ret = fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_CHIP_ID2, &chip_id[1]);
...
static int get_ic_types_by_chipid(short chipid)
{
	...
	FTS_TEST_INFO("chip_id:%4x\n", (fts_data_mi->chipid & 0xffff));
	snprintf(ic_name, 30, "FT%4x", (fts_data_mi->chipid & 0xffff));
	return fts_ic_table_get_ic_code_from_ic_name_mi(ic_name);
}
...
ssize_t fts_tp_selftest_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	fts_data_mi->is_tp_testing = true;

	if (!fts_data_mi->client || count > sizeof(tmp)) {
	...
	ic_type = get_ic_types_by_chipid(fts_data_mi->chipid);
	if (ic_type == 0) {
		retval = -EFAULT;
		fts_test_result = SELFTEST_INVALID;
		goto out;
	}
	retval = init_test_funcs_mi(ic_type);
	if (retval < 0) {
		retval = -EFAULT;
		fts_test_result = SELFTEST_INVALID;
		goto out;
...
test_err:
	fts_data_mi->is_tp_testing = false;
	return retval;
}
...
static int32_t c_show(struct seq_file *m, void *v)
{
	...
	/* 0xEE = 1, disable cb  */
	retval = write_reg_mi(FACTORY_REG_AUTO_CAL_FLAG, 0x01);
	if (retval) {
...
int fts_test_init_mi(struct i2c_client *client)
{
	int ret = 0;
	...
	fts_data_mi->tp_selftest_proc = proc_create("tp_selftest", 0644, NULL, &tp_selftest_ops);
	if (!fts_data_mi->tp_selftest_proc)
		FTS_TEST_ERROR("[focal] %s() - tp_selftest proc create failed.", __func__);

	fts_data_mi->tp_data_dump_proc = proc_create("tp_data_dump", 0444, NULL, &fts_datadump_fops);
	FTS_TEST_ERROR("create file node of tp_data_dump");
    if (!fts_data_mi->tp_data_dump_proc)
...
int fts_test_exit_mi(struct i2c_client *client)
{
	FTS_TEST_FUNC_ENTER();

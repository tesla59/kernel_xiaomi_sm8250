...
static ssize_t fts_gesture_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	fts_i2c_read_reg_mi(client, FTS_REG_GESTURE_EN, &val);
...
static int fts_gesture_read_buffer(struct i2c_client *client, u8 *buf, int read_bytes)
{
	...
	if (read_bytes <= I2C_BUFFER_LENGTH_MAXINUM) {
		ret = fts_i2c_read_mi(client, buf, 1, buf, read_bytes);
	} else {
		ret = fts_i2c_read_mi(client, buf, 1, buf, I2C_BUFFER_LENGTH_MAXINUM);
		remain_bytes = read_bytes - I2C_BUFFER_LENGTH_MAXINUM;
		for (i = 1; remain_bytes > 0; i++) {
			if (remain_bytes <= I2C_BUFFER_LENGTH_MAXINUM)
				ret = fts_i2c_read_mi(client, buf, 0, buf + I2C_BUFFER_LENGTH_MAXINUM * i, remain_bytes);
			else
				ret =
					fts_i2c_read_mi(client, buf, 0, buf + I2C_BUFFER_LENGTH_MAXINUM * i,
...
int fts_gesture_readdata_mi(struct fts_ts_data_mi *ts_data)
{
	...
	ret = fts_i2c_read_reg_mi(client, FTS_REG_GESTURE_EN, &state);
	...
	ret = fts_i2c_read_mi(client, buf, 1, buf, FTS_GESTRUE_POINTS_HEADER);
	if (ret < 0) {
...
void fts_gesture_recovery_mi(struct i2c_client *client)
{
	if ((ENABLE == fts_gesture_data.mode) && (ENABLE == fts_gesture_data.active)) {
		...
		fts_i2c_write_reg_mi(client, 0xD1, 0xff);
		fts_i2c_write_reg_mi(client, 0xD2, 0xff);
		fts_i2c_write_reg_mi(client, 0xD5, 0xff);
		fts_i2c_write_reg_mi(client, 0xD6, 0xff);
		fts_i2c_write_reg_mi(client, 0xD7, 0xff);
		fts_i2c_write_reg_mi(client, 0xD8, 0xff);
		fts_gesture_reg_write_mi(client, FTS_REG_GESTURE_DOUBLETAP_ON, true);
		if (FTS_FOD)
			fts_fod_reg_write_mi(client, FTS_REG_GESTURE_DOUBLETAP_ON, true);
	}
}
...
void fts_fod_recovery_mi(struct i2c_client *client)
{
	...
	fts_fod_reg_write_mi(client, FTS_REG_GESTURE_FOD_ON, true);
}
...
int fts_fod_reg_write_mi(struct i2c_client *client, u8 mask, bool enable)
{
	...
	for (i = 0; i < 5; i++) {
		fts_i2c_read_reg_mi(client, FTS_REG_GESTURE_SUPPORT, &reg_value);
		if (enable)
			reg_value |= mask;
		else
			reg_value &= ~mask;
		fts_i2c_write_reg_mi(client, FTS_REG_GESTURE_SUPPORT, reg_value);
		msleep(1);
		fts_i2c_read_reg_mi(client, FTS_REG_GESTURE_SUPPORT, &state);
		if (state == reg_value)
			break;
	}
...
int fts_gesture_reg_write_mi(struct i2c_client *client, u8 mask, bool enable)
{
	...
	for (i = 0; i < 5; i++) {
		fts_i2c_read_reg_mi(client, FTS_REG_GESTURE_EN, &reg_value);
		if (enable)
			reg_value |= mask;
		else
			reg_value &= ~mask;
		fts_i2c_write_reg_mi(client, FTS_REG_GESTURE_EN, reg_value);
		msleep(1);
		fts_i2c_read_reg_mi(client, FTS_REG_GESTURE_EN, &state);
		if (state == reg_value)
			break;
	}
...
*****************************************************************************/
int fts_gesture_suspend_mi(struct i2c_client *client)
{
	int ret;
	...
	ret = fts_gesture_reg_write_mi(client, FTS_REG_GESTURE_DOUBLETAP_ON, true);
	if (ret) {
	...
	if (FTS_FOD) {
		ret = fts_fod_reg_write_mi(client, FTS_REG_GESTURE_DOUBLETAP_ON, true);
		if (ret) {
			FTS_ERROR("[GESTURE]Enter into gesture(suspend) failed!\n");
			fts_gesture_data.active = DISABLE;
			return -EIO;
...
int fts_gesture_resume_mi(struct i2c_client *client)
{
	...
	ret = fts_gesture_reg_write_mi(client, FTS_REG_GESTURE_DOUBLETAP_ON, false);
	if (ret) {
		FTS_ERROR("[GESTURE]Resume from gesture failed!\n");
		return -EIO;
	}
	if (FTS_FOD) {
		ret = fts_fod_reg_write_mi(client, FTS_REG_GESTURE_DOUBLETAP_ON, false);
		if (ret) {

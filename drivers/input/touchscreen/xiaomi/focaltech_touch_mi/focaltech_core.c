...
#include <linux/fb.h>
#include <drm/xiaomi/drm_notifier_mi.h>
...
*****************************************************************************/
struct fts_ts_data_mi *fts_data_mi;
#if FTS_CHARGER_EN
...
static int fts_read_and_report_foddata(struct fts_ts_data_mi *data);
#ifdef CONFIG_TOUCHSCREEN_XIAOMI_TOUCHFEATURE
static int fts_read_palm_data(void);
static int fts_palm_enable(struct fts_ts_data_mi *fts_data_mi, int on);
#endif

struct device *fts_get_dev_mi(void)
{
	if (!fts_data_mi)
		return NULL;
	else
		return &(fts_data_mi->client->dev);
}
/*****************************************************************************
*  Name: fts_wait_tp_to_valid_mi
...
*****************************************************************************/
int fts_wait_tp_to_valid_mi(struct i2c_client *client)
{
	int ret = 0;
	int cnt = 0;
	u8 reg_value = 0;
	u8 chip_id = fts_data_mi->ic_info.ids.chip_idh;
...
#endif
	do {
		ret = fts_i2c_read_reg_mi(client, FTS_REG_CHIP_ID, &reg_value);
...
***********************************************************************/
static int fts_get_chip_types(struct fts_ts_data_mi *ts_data, u8 id_h, u8 id_l, bool fw_valid)
{
	int i = 0;
	struct ft_chip_mi_t ctype[] = FTS_CHIP_TYPE_MAPPING;
	u32 ctype_entries = sizeof(ctype) / sizeof(struct ft_chip_mi_t);
...
*  Return: return 0 if success, otherwise return error code
*****************************************************************************/
static int fts_get_ic_information(struct fts_ts_data_mi *ts_data)
{
	int ret = 0;
	int cnt = 0;
	u8 chip_id[2] = { 0 };
	...
	do {
		ret = fts_i2c_read_reg_mi(client, FTS_REG_CHIP_ID, &chip_id[0]);
		ret = fts_i2c_read_reg_mi(client, FTS_REG_CHIP_ID2, &chip_id[1]);
	...
	if ((cnt * INTERVAL_READ_REG) >= TIMEOUT_READ_REG) {
		...
		if (ts_data->ic_info.hid_supported) {
			fts_i2c_hid2std_mi(client);
		}

		id_cmd[0] = FTS_CMD_START1;
		id_cmd[1] = FTS_CMD_START2;
		ret = fts_i2c_write_mi(client, id_cmd, 2);
		if (ret < 0) {
		...
		ret = fts_i2c_read_mi(client, id_cmd, id_cmd_len, chip_id, 2);
		if ((ret < 0) || (0x0 == chip_id[0]) || (0x0 == chip_id[1])) {
			FTS_ERROR("read boot id fail");
			return -EIO;
		}
		ret = fts_get_chip_types(ts_data, chip_id[0], chip_id[1], FTS_INVALID);
		if (ret < 0) {
...
/*****************************************************************************
*  Name: fts_tp_state_recovery_mi
*  Brief: Need execute this function when reset
*  Input:
*  Output:
*  Return:
*****************************************************************************/
void fts_tp_state_recovery_mi(struct i2c_client *client)
{
	FTS_FUNC_ENTER();
	/* wait tp stable */
	fts_wait_tp_to_valid_mi(client);
	/* recover TP charger state 0x8B */
	/* recover TP glove state 0xC0 */
	/* recover TP cover state 0xC1 */
	fts_ex_mode_recovery_mi(client);
	/* recover TP gesture state 0xD0 */
#if FTS_GESTURE_EN
	fts_gesture_recovery_mi(client);
#endif
	FTS_FUNC_EXIT();
}

/*****************************************************************************
*  Name: fts_reset_proc_mi
*  Brief: Execute reset operation
*  Input: hdelayms - delay time unit:ms
*  Output:
*  Return:
*****************************************************************************/
int fts_reset_proc_mi(int hdelayms)
{
	u8 chip_id[2] = { 0 };
	int read_time = 10,ret = 0;
	struct i2c_client *client = fts_data_mi->client;

	FTS_FUNC_ENTER();
	gpio_direction_output(fts_data_mi->pdata->reset_gpio, 0);
	mdelay(5);
	gpio_direction_output(fts_data_mi->pdata->reset_gpio, 1);
	if (hdelayms) {
		mdelay(hdelayms);
	}
	while(read_time){
	...
	if (FTS_FOD)
		fts_fod_recovery_mi(fts_data_mi->client);
...
/*****************************************************************************
*  Name: fts_irq_disable_mi
*  Brief: disable irq
*  Input:
*  Output:
*  Return:
*****************************************************************************/
void fts_irq_disable_mi(void)
{
	unsigned long irqflags;

	FTS_FUNC_ENTER();
	spin_lock_irqsave(&fts_data_mi->irq_lock, irqflags);

	if (!fts_data_mi->irq_disabled) {
		disable_irq_nosync(fts_data_mi->irq);
		fts_data_mi->irq_disabled = true;
	}

	spin_unlock_irqrestore(&fts_data_mi->irq_lock, irqflags);
	FTS_FUNC_EXIT();
}
...
/*****************************************************************************
*  Name: fts_irq_enable_mi
*  Brief: enable irq
*  Input:
*  Output:
*  Return:
*****************************************************************************/
void fts_irq_enable_mi(void)
{
	...
	if (fts_data_mi->irq_disabled) {
		enable_irq(fts_data_mi->irq);
		FTS_INFO("irq is enabled\n");
		fts_data_mi->irq_disabled = false;
	}
	spin_unlock_irqrestore(&fts_data_mi->irq_lock, irqflags);
...
* Power Control
*****************************************************************************/
static int fts_power_source_init(struct fts_ts_data_mi *data)
{
	int ret = 0;
...
static int fts_power_source_release(struct fts_ts_data_mi *data)
{
	return 0;
}

static int fts_power_source_ctrl(struct fts_ts_data_mi *data, int enable)
{
	int ret = 0;
...
*  Return:
*****************************************************************************/
static int fts_pinctrl_init(struct fts_ts_data_mi *ts)
{
	int ret = 0;
...
static int fts_pinctrl_select_normal(struct fts_ts_data_mi *ts)
{
	int ret = 0;
...
static int fts_pinctrl_select_suspend(struct fts_ts_data_mi *ts)
{
	int ret = 0;
...
static int fts_pinctrl_select_release(struct fts_ts_data_mi *ts)
{
	int ret = 0;
...
static void fts_show_touch_buffer(u8 *buf, int point_num)
{
	...
	if (len > (fts_data_mi->pnt_buf_size - 3)) {
		len = fts_data_mi->pnt_buf_size - 3;
	} else if (len == 0) {
		len += FTS_ONE_TCH_LEN;
	}
...
static void fts_release_all_finger(void)
{
	struct input_dev *input_dev = fts_data_mi->input_dev;
	...
	FTS_FUNC_ENTER();
	if (FTS_FOD) {
		fts_data_mi->finger_in_fod = false;
		/* fts_data_mi->overlap_area = 0; */
		fod_overlap_aera = 0;
	}
#if FTS_MT_PROTOCOL_B_EN
	for (finger_count = 0; finger_count < fts_data_mi->pdata->max_touch_number; finger_count++) {
...
#if FTS_MT_PROTOCOL_B_EN
static int fts_input_report_b(struct fts_ts_data_mi *data)
{
	int i = 0;
	int uppoint = 0;
	int touchs = 0;
	...
	struct ts_event_mi *events = data->events;
...
static int fts_read_and_report_foddata(struct fts_ts_data_mi *data)
{
	...
	ret = fts_i2c_read_mi(data->client, buf, 1, buf + 1, 9);
	x = (buf[5] << 8) | buf[6];
	y = (buf[7] << 8) | buf[8];
	z = buf[4];
...
*****************************************************************************/
static int fts_read_touchdata(struct fts_ts_data_mi *data)
{
	int ret = 0;
	int i = 0;
	u8 pointid;
...
#if FTS_GESTURE_EN
	if (FTS_FOD && (fts_read_and_report_foddata(data) == 0)) {
		if (irq_num == 1)
			FTS_INFO("succuss to get fod data in irq handler");
		ret = fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_INT_ACK, &reg_value);
		if (ret < 0)
			FTS_ERROR("read int ack error");
		else
			return 1;
	} else if (0 == fts_gesture_readdata_mi(data)) {
	...
	buf[0] = 0x00;
#ifndef CONFIG_MACH_XIAOMI_SM8250
	ret = fts_i2c_read_mi(data->client, buf, 1, buf, 16);
	if (buf[15] == 0xFF) {
		ret = fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_INT_ACK, &reg_value);
		if (ret < 0) {
			FTS_ERROR("read int ack error");
		...
		buf[0] = 0x00;
#endif
		ret = fts_i2c_read_mi(data->client, buf, 1, buf, data->pnt_buf_size);
		if (ret < 0) {
	...
	if ((data->point_num == 0x0F) && (buf[1] == 0xFF) && (buf[2] == 0xFF)
		...
		fts_tp_state_recovery_mi(client);
		return -EIO;
	}
...
*  Return:
*****************************************************************************/
static void fts_report_event(struct fts_ts_data_mi *data)
{
#if FTS_MT_PROTOCOL_B_EN
...
{
	int ret = 0;
	struct fts_ts_data_mi *ts_data = (struct fts_ts_data_mi *)data;
...
*****************************************************************************/
static int fts_irq_registration(struct fts_ts_data_mi *ts_data)
{
	int ret = 0;
	struct fts_ts_platform_data_mi *pdata = ts_data->pdata;
...
{
	struct fts_ts_data_mi *ts_data = input_get_drvdata(dev);
...
*  Return:
*****************************************************************************/
static int fts_input_init(struct fts_ts_data_mi *ts_data)
{
	int ret = 0;
	struct fts_ts_platform_data_mi *pdata = ts_data->pdata;
	...
	ts_data->events = (struct ts_event_mi *)kzalloc(point_num * sizeof(struct ts_event_mi), GFP_KERNEL);
	if (!ts_data->events) {
...
*  Return: return 0 if succuss
*****************************************************************************/
static int fts_gpio_configure(struct fts_ts_data_mi *data)
{
	int ret = 0;
...
static int fts_read_palm_data(void)
{
	...
	/* FTS_FUNC_ENTER(); */
	ret = fts_i2c_read_reg_mi(fts_data_mi->client, 0x9b, &reg_value);
	if (ret < 0) {
...
static int fts_palm_enable(struct fts_ts_data_mi *fts_data_mi, int on)
{
	...
	if (on)
		retval = fts_i2c_write_reg_mi(fts_data_mi->client, 0x9A, palm_on);
	else
		retval = fts_i2c_write_reg_mi(fts_data_mi->client, 0x9A, palm_off);
...
static int fts_palm_sensor_write(int value)
{
	...
	if (!fts_data_mi)
		return -EINVAL;
	fts_data_mi->palm_sensor_switch = value;
	XIAOMI_TP_DEBUG("-----%s-----value=%d\n", __func__, value);
	if (fts_data_mi->dev_pm_suspend) {
		FTS_ERROR("%s tp has dev_pm_suspend\n", __func__);
		fts_data_mi->palm_sensor_changed = false;
		return 0;
	}
	if (!fts_data_mi->suspended) {
		FTS_ERROR("-----%s-----suspended----value=%d\n",__func__,value);
		ret = fts_palm_enable(fts_data_mi, value);
		if (!ret)
			fts_data_mi->palm_sensor_changed = true;
	}
	FTS_FUNC_EXIT();
	return ret;
}
...
static void fts_init_touchmode_data(void)
{
	...
	/* sensivity */
	ret = fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_SENSIVITY, &reg_value);
	if (ret < 0) {
	...
	/*  Tolerance */
	ret = fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_THDIFF, &reg_value);
	if (ret < 0) {
...
static void fts_update_touchmode_data(int mode)
{
	...
	case Touch_Game_Mode:
		...
		if (temp_value == 1) {
			ret = fts_i2c_write_reg_mi(fts_data_mi->client, FTS_REG_MONITOR_MODE, 0);
			if (ret < 0)
				FTS_ERROR("disable monitor mode error, ret=%d\n", ret);
			fts_data_mi->gamemode_enabled = true;
		} else {
			/*restore touch parameters */
			ret = fts_i2c_write_reg_mi(fts_data_mi->client, FTS_REG_MONITOR_MODE, 1);
			if (ret < 0)
				FTS_ERROR("restore monitor mode error, ret=%d\n", ret);
			ret = fts_i2c_write_reg_mi(fts_data_mi->client, FTS_REG_SENSIVITY, (u8)xiaomi_touch_interfaces.touch_mode[Touch_UP_THRESHOLD][GET_DEF_VALUE]);
			if (ret < 0)
				FTS_ERROR("restore sensitivity error, ret=%d\n", ret);
			ret = fts_i2c_write_reg_mi(fts_data_mi->client, FTS_REG_THDIFF, (u8)xiaomi_touch_interfaces.touch_mode[Touch_Tolerance][GET_DEF_VALUE]);
			if (ret < 0)
				FTS_ERROR("restore touch smooth error, ret=%d\n", ret);
			ret = fts_i2c_write_reg_mi(fts_data_mi->client, FTS_REG_EDGE_FILTER_ORIENTATION, (u8)xiaomi_touch_interfaces.touch_mode[Touch_Panel_Orientation][GET_DEF_VALUE]);
			if (ret < 0)
				FTS_ERROR("restore orientation error, ret=%d\n", ret);
			ret = fts_i2c_write_reg_mi(fts_data_mi->client, FTS_REG_EDGE_FILTER_LEVEL, (u8)xiaomi_touch_interfaces.touch_mode[Touch_Edge_Filter][GET_DEF_VALUE]);
			if (ret < 0)
				FTS_ERROR("restore orientation error, ret=%d\n", ret);
			fts_data_mi->gamemode_enabled = false;
		}
		break;
	case Touch_Active_MODE:
		break;
	case Touch_UP_THRESHOLD:
			if (fts_data_mi->gamemode_enabled) {
				ret = fts_i2c_write_reg_mi(fts_data_mi->client, FTS_REG_SENSIVITY, temp_value);
				if (ret < 0)
	...
	case Touch_Tolerance:
			if (fts_data_mi->gamemode_enabled) {
				ret = fts_i2c_write_reg_mi(fts_data_mi->client, FTS_REG_THDIFF, temp_value);
				if (ret < 0)
	...
	case Touch_Panel_Orientation:
			...
			if (temp_value == 0 || temp_value == 2) {
				ret = fts_i2c_write_reg_mi(fts_data_mi->client, FTS_REG_EDGE_FILTER_ORIENTATION, 0);
				if (ret < 0)
					FTS_ERROR("write orientation error, ret=%d\n", ret);
			}
			if (temp_value == 1) {
				ret = fts_i2c_write_reg_mi(fts_data_mi->client, FTS_REG_EDGE_FILTER_ORIENTATION, 1);
				if (ret < 0)
					FTS_ERROR("write orientation error, ret=%d\n", ret);
			}
			if (temp_value == 3) {
				ret = fts_i2c_write_reg_mi(fts_data_mi->client, FTS_REG_EDGE_FILTER_ORIENTATION, 2);
				if (ret < 0)
					FTS_ERROR("write orientation error, ret=%d\n", ret);
			}
		break;
	case Touch_Edge_Filter:
			if (fts_data_mi->gamemode_enabled) {
				 ...
				 **/
				ret = fts_i2c_write_reg_mi(fts_data_mi->client, FTS_REG_EDGE_FILTER_LEVEL, temp_value + 1);
				if (ret < 0)
...
static int fts_set_cur_value(int mode, int value)
{
	...
	if (mode == Touch_Fod_Enable && fts_data_mi && value >= 0) {
		fts_data_mi->fod_status = value;
		return 0;
	}
	if (mode == Touch_Aod_Enable && fts_data_mi && value >= 0) {
		fts_data_mi->aod_status = value;
		return 0;
	}
...
*  Return: return 0 if succuss, otherwise return error code
*****************************************************************************/
static int fts_get_dt_coords(struct device *dev, char *name, struct fts_ts_platform_data_mi *pdata)
{
	int ret = 0;
...
*  Return:
*****************************************************************************/
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data_mi *pdata)
{
	int ret = 0;
	struct device_node *np = dev->of_node;
	u32 temp_val;
...
{
	struct fts_ts_data_mi *ts_data = dev_get_drvdata(dev);
					...
					struct device_attribute *attr, char *buf)
{
	...
	fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_MONITOR_MODE, &monitor_mode);
	fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_REPORT_RATE, &report_rate);
	fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_SENSIVITY, &sensitivity);
	fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_THDIFF, &thdiff);
	fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_EDGE_FILTER_ORIENTATION, &filter_ori);
	fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_EDGE_FILTER_LEVEL, &filter_level);
...
static ssize_t fts_lockdown_info_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	cnt =
		snprintf(tmp, TP_INFO_MAX_LENGTH,
			...
			fts_data_mi->lockdown_info[0], fts_data_mi->lockdown_info[1],
			fts_data_mi->lockdown_info[2], fts_data_mi->lockdown_info[3], fts_data_mi->lockdown_info[4],
			fts_data_mi->lockdown_info[5], fts_data_mi->lockdown_info[6], fts_data_mi->lockdown_info[7]);
...
static ssize_t fts_fw_version_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	fts_i2c_read_reg_mi(fts_data_mi->client, FTS_REG_FW_VER, &major_version);
	fts_i2c_read_reg_mi(fts_data_mi->client, 0xAD, &minor_version);
...
static void tpdbg_suspend(struct fts_ts_data_mi *ts_data, bool enable)
{
	if (enable)
...
{
	struct fts_ts_data_mi *ts_data = file->private_data;
...
{
	struct fts_ts_data_mi *ts_data = container_of(work, struct fts_ts_data_mi, power_supply_work);
	int is_usb_exist;
...
{
	struct fts_ts_data_mi *ts_data = container_of(nb, struct fts_ts_data_mi, power_supply_notifier);
...
{
	struct fts_ts_data_mi *ts_data = container_of(nb, struct fts_ts_data_mi, bl_notif);
	unsigned int blank;
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	int *blank;
	struct fts_ts_data_mi *fts_data = container_of(self, struct fts_ts_data_mi, fb_notif);
...
{
	struct fts_ts_data_mi *data = container_of(handler,
						struct fts_ts_data_mi,
						early_suspend);
...
static int check_is_focal_touch(struct fts_ts_data_mi *ts_data)
{
	int ret = false;
	u8 cmd[4] = { 0 };
	u32 cmd_len = 0;
	...
	fts_reset_proc_mi(10);
	cmd[0] = FTS_CMD_START1;
	cmd[1] = FTS_CMD_START2;
	ret = fts_i2c_write_mi(ts_data->client, cmd, 2);
	...
	ret = fts_i2c_read_mi(ts_data->client, cmd, cmd_len, val, 2);
	if (ret < 0) {
...
{
	if (fts_data_mi)
		return fts_data_mi->lockdown_info[0];
	else
		return 0;
}
...
{
	if (fts_data_mi)
		return fts_data_mi->lockdown_info[2];
	else
		return 0;
}
...
{
	if (fts_data_mi)
		return fts_data_mi->lockdown_info[1];
	else
		return 0;
}
...
{
	int ret = 0;
	struct fts_ts_platform_data_mi *pdata;
	struct fts_ts_data_mi *ts_data;
...
#if FTS_APK_NODE_EN
	ret = fts_create_apk_debug_channel_mi(ts_data);
	if (ret) {
	...
	ret = fts_create_sysfs_mi(client);
	if (ret) {
...
#if FTS_POINT_REPORT_CHECK_EN
	ret = fts_point_report_check_init_mi(ts_data);
	if (ret) {
	...
	ret = fts_ex_mode_init_mi(client);
	if (ret) {
		FTS_ERROR("init glove/cover/charger fail");
		goto err_debugfs_create;
	}
#if FTS_GESTURE_EN
	ret = fts_gesture_init_mi(ts_data);
	if (ret) {
...
#if FTS_ESDCHECK_EN
	ret = fts_esdcheck_init_mi(ts_data);
	if (ret) {
...
{
	struct fts_ts_data_mi *ts_data = i2c_get_clientdata(client);
...
#if FTS_POINT_REPORT_CHECK_EN
	fts_point_report_check_exit_mi(ts_data);
#endif

#if FTS_APK_NODE_EN
	fts_release_apk_debug_channel_mi(ts_data);
#endif

#if FTS_SYSFS_NODE_EN
	fts_remove_sysfs_mi(client);
#endif
	destroy_workqueue(ts_data->event_wq);
	fts_ex_mode_exit_mi(client);

#if FTS_AUTO_UPGRADE_EN
	fts_fwupg_exit_mi(ts_data);
#endif

#if FTS_TEST_EN
	fts_test_exit_mi(client);
#endif

#if FTS_ESDCHECK_EN
	fts_esdcheck_exit_mi(ts_data);
#endif
...
/*
	...
	if (ts_data->palm_sensor_switch) {
		...
		fts_data_mi->palm_sensor_switch = false;
		ts_data->palm_sensor_changed = true;
	}
#endif
*/
#if FTS_ESDCHECK_EN
	fts_esdcheck_suspend_mi();
#endif
	fts_irq_disable_sync();

	ts_data->fod_point_released = false;
#if FTS_GESTURE_EN
	if (fts_gesture_suspend_mi(ts_data->client) == 0) {
		if(ts_data->fod_status == -1 || ts_data->fod_status == 100){
			fts_fod_reg_write_mi(ts_data->client, FTS_REG_GESTURE_FOD_ON, false);
		}
	...
	if (FTS_FOD) {
		if (ts_data->aod_status || (ts_data->fod_status != -1 && ts_data->fod_status != 100)) {
			ret = fts_fod_reg_write_mi(ts_data->client, FTS_REG_GESTURE_FOD_ON, true);
			if (ret < 0) {
				FTS_ERROR("%s fts_fod_reg_write failed, enter sleep mode!\n", __func__);
				goto sleep_mode;
			}

			fts_gesture_reg_write_mi(ts_data->client, FTS_REG_GESTURE_DOUBLETAP_ON, true);
			if (ret < 0) {
...
sleep_mode:
	...
	ret = fts_i2c_write_reg_mi(ts_data->client, FTS_REG_POWER_MODE, FTS_REG_POWER_MODE_SLEEP_VALUE);
	if (ret < 0)
		FTS_ERROR("set TP to sleep mode fail, ret=%d", ret);
	ts_data->suspended = true;
release_finger:
	mutex_lock(&fts_data_mi->report_mutex);
	fts_release_all_finger();
	mutex_unlock(&fts_data_mi->report_mutex);
	if (FTS_FOD)
		mutex_unlock(&ts_data->fod_mutex);
	FTS_FUNC_EXIT();
	return 0;
...
static int fts_ts_resume(struct device *dev)
{
	...
	if (FTS_FOD && ts_data->pdata->reset_when_resume &&
	    !ts_data->finger_in_fod && !ts_data->fod_finger_skip) {
		fts_reset_proc_mi(50);
		mutex_lock(&fts_data_mi->report_mutex);
		fts_release_all_finger();
		mutex_unlock(&fts_data_mi->report_mutex);
	} else if (ts_data->pdata->reset_when_resume) {
		fts_reset_proc_mi(50);
		mutex_lock(&fts_data_mi->report_mutex);
		fts_release_all_finger();
		mutex_unlock(&fts_data_mi->report_mutex);
	...
	if (FTS_FOD && fts_data_mi->finger_in_fod) {
		fts_fod_reg_write_mi(ts_data->client, FTS_REG_GESTURE_FOD_NO_CAL, true);
	}

	fts_tp_state_recovery_mi(ts_data->client);

#if FTS_ESDCHECK_EN
	fts_esdcheck_resume_mi();
#endif

#if FTS_GESTURE_EN
	if (fts_gesture_resume_mi(ts_data->client) == 0) {
	...
	if (FTS_FOD)
		fts_gesture_reg_write_mi(ts_data->client, FTS_REG_GESTURE_DOUBLETAP_ON, false);
	ts_data->suspended = false;
	fts_irq_enable_mi();
#ifdef CONFIG_TOUCHSCREEN_XIAOMI_TOUCHFEATURE
	if (ts_data->palm_sensor_switch) {
...
static int fts_pm_suspend(struct device *dev)
{
	...
	if (FTS_FOD || (!FTS_FOD && ts_data->lpwg_mode)) {
...
static void fts_resume_work(struct work_struct *work)
{
	struct fts_ts_data_mi *ts;
	ts = container_of(work, struct fts_ts_data_mi, resume_work);
...
{
	struct fts_ts_data_mi *ts;
	ts = container_of(work, struct fts_ts_data_mi, suspend_work);

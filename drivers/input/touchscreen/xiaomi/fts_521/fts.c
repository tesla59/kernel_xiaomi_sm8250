...
#define TOUCHSCREEN_FOD \
	(socinfo_get_platform_type() == HW_PLATFORM_CAS || \
	 socinfo_get_platform_type() == HW_PLATFORM_CMI || \
...
#define SECURE_TOUCH \
	(socinfo_get_platform_type() == HW_PLATFORM_CAS || \
	 ...
	 socinfo_get_platform_type() == HW_PLATFORM_UMI)
#else
#define TOUCHSCREEN_FOD (0)
#define SECURE_TOUCH (0)
#endif
...
static DEVICE_ATTR(secure_touch_enable, (S_IRUGO | S_IWUSR | S_IWGRP), fts_secure_touch_enable_show,  fts_secure_touch_enable_store);
static DEVICE_ATTR(secure_touch, (S_IRUGO | S_IWUSR | S_IWGRP), fts_secure_touch_show,  NULL);
/**@}*/
/**@}*/
					    ...
					    unsigned char *event)
{
	...
	u8 touchType;
	int area_size;
	if (!TOUCHSCREEN_FOD && !info->resume_bit)
		goto no_report;
					    ...
					    unsigned char *event)
{
	...
	if (TOUCHSCREEN_FOD && (event[1] == 0xb5)) {
...
static void fts_init_touchmode_data(void)
{
	...
	xiaomi_touch_interfaces.touch_mode[Touch_Aim_Sensitivity][GET_MAX_VALUE] = 15;
	xiaomi_touch_interfaces.touch_mode[Touch_Aim_Sensitivity][GET_MIN_VALUE] = 0;

	/*	Wgh Max */
	xiaomi_touch_interfaces.touch_mode[Touch_Tap_Stability][GET_DEF_VALUE] = 14;
	xiaomi_touch_interfaces.touch_mode[Touch_Tap_Stability][GET_CUR_VALUE] = 14;
	xiaomi_touch_interfaces.touch_mode[Touch_Tap_Stability][SET_CUR_VALUE] = 14;
	xiaomi_touch_interfaces.touch_mode[Touch_Tap_Stability][GET_MAX_VALUE] = 15;
	...
	xiaomi_touch_interfaces.touch_mode[Touch_Expert_Mode][GET_MAX_VALUE] = 2;
...
static int fts_set_fod_status(int value)
{
	...
	if (fts_info->fod_status == 2) {
		...
		res = fts_write_521(gesture_cmd, ARRAY_SIZE(gesture_cmd));
		if (res < OK)
			MI_TOUCH_LOGE(1, "%s %s: enter gesture and longpress failed! ERROR %08X recovery in senseOff...\n",
			tag, __func__, res);
		else
...
int fts_palm_sensor_write(int value)
{
	...
	if (SECURE_TOUCH)
		mutex_lock(&scr_info->palm_lock);
	if (SECURE_TOUCH && atomic_read(&scr_info->st_enabled)) {
		if (!scr_info->scr_delay.palm_pending) {
...
static int fts_pm_suspend(struct device *dev)
{
	...
	else if (device_may_wakeup(dev) && info->gesture_enabled)
		enable_irq_wake(info->client->irq);

...
*/
int fts_read_521(u8 *outBuf, int byteToRead)
{
	int ret = -1;
	int retry = 0;
...
* @return OK if success or an error code which specify the type of error encountered
*/
int fts_write_521(u8 *cmd, int cmdLength)
{
	int ret = -1;
	int retry = 0;
...
int fts_read_dma_safe(u8 *outBuf, int byteToRead)
{
	...
	ret = fts_read_521(finalBuf, byteToRead);
...
int fts_write_dma_safe(u8 *cmd, int cmdLength)
{
	...
	ret = fts_write_521(finalBuf, cmdLength);
...
int fts_read_dma_safe(u8 *outBuf, int byteToRead)
{
	return fts_read_521(outBuf, byteToRead);
}
int fts_writeRead_dma_safe(u8 *cmd, int cmdLength, u8 *outBuf, int byteToRead)
{
...
int fts_write_dma_safe(u8 *cmd, int cmdLength)
{
	return fts_write_521(cmd, cmdLength);
}
#endif
		  ...
		  int dataSize)
{
	...
	if (addrSize <= sizeof(u64)) {
		...
		while (remaining > 0) {
			...
			if (fts_write_521(finalCmd, 1 + addrSize + toWrite) < OK) {
				logError(1, "%s %s: ERROR %08X \n", tag,
			       ...
			       int dataSize)
{
	...
	while (remaining > 0) {
		...
		if (fts_write_521(finalCmd1, 1 + addrSize1) < OK) {
		...
		if (fts_write_521(finalCmd2, 1 + addrSize2 + toWrite) < OK) {
			logError(1,

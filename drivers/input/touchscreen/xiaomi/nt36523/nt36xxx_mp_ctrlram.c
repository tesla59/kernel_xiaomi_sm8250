...
static int32_t nvt_save_rawdata_to_csv(int32_t *rawdata, uint8_t x_ch, uint8_t y_ch, const char *file_path, uint32_t offset, enum test_type_36523 type)
{
	...
	struct test_buf_36523 *tbuf = ts->testdata;
...
static int32_t nvt_polling_hand_shake_status(void)
{
	...
	if (i >= retry) {
		...
		nvt_set_page_36523(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_HOST_CMD);
...
static int32_t nvt_read_baseline(int32_t *xdata)
{
	...
	nvt_read_mdata_36523(ts->mmap->BASELINE_ADDR, ts->mmap->BASELINE_BTN_ADDR);

	nvt_get_mdata_36523(xdata, &x_num, &y_num);
...
static int32_t nvt_read_CC(int32_t *xdata)
{
	...
	if (nvt_get_fw_pipe_36523() == 0)
		nvt_read_mdata_36523(ts->mmap->DIFF_PIPE1_ADDR, ts->mmap->DIFF_BTN_PIPE1_ADDR);
	else
		nvt_read_mdata_36523(ts->mmap->DIFF_PIPE0_ADDR, ts->mmap->DIFF_BTN_PIPE0_ADDR);
...
static int32_t nvt_read_pen_baseline(void)
{
	...
	nvt_read_get_num_mdata_36523(ts->mmap->PEN_2D_BL_TIP_X_ADDR, RawData_PenTipX_Raw, ts->x_num * ts->y_gang_num);
	nvt_read_get_num_mdata_36523(ts->mmap->PEN_2D_BL_TIP_Y_ADDR, RawData_PenTipY_Raw, ts->x_gang_num * ts->y_num);
	nvt_read_get_num_mdata_36523(ts->mmap->PEN_2D_BL_RING_X_ADDR, RawData_PenRingX_Raw, ts->x_num * ts->y_gang_num);
	nvt_read_get_num_mdata_36523(ts->mmap->PEN_2D_BL_RING_Y_ADDR, RawData_PenRingY_Raw, ts->x_gang_num * ts->y_num);
...
static void nvt_enable_noise_collect(int32_t frame_num)
{
	...
	buf[3] = frame_num;
	buf[4] = 0x00;
	CTP_SPI_WRITE_36523(ts->client, buf, 5);
}
...
static int32_t nvt_read_fw_noise(int32_t *xdata)
{
	...
	if (ts->pen_support) {
		// get pen noise data
		nvt_read_get_num_mdata_36523(ts->mmap->PEN_2D_DIFF_TIP_X_ADDR, RawData_PenTipX_DiffMax, ts->x_num * ts->y_gang_num);
		nvt_read_get_num_mdata_36523(ts->mmap->PEN_2D_RAW_TIP_X_ADDR, RawData_PenTipX_DiffMin, ts->x_num * ts->y_gang_num);
		nvt_read_get_num_mdata_36523(ts->mmap->PEN_2D_DIFF_TIP_Y_ADDR, RawData_PenTipY_DiffMax, ts->x_gang_num * ts->y_num);
		nvt_read_get_num_mdata_36523(ts->mmap->PEN_2D_RAW_TIP_Y_ADDR, RawData_PenTipY_DiffMin, ts->x_gang_num * ts->y_num);
		nvt_read_get_num_mdata_36523(ts->mmap->PEN_2D_DIFF_RING_X_ADDR, RawData_PenRingX_DiffMax, ts->x_num * ts->y_gang_num);
		nvt_read_get_num_mdata_36523(ts->mmap->PEN_2D_RAW_RING_X_ADDR, RawData_PenRingX_DiffMin, ts->x_num * ts->y_gang_num);
		nvt_read_get_num_mdata_36523(ts->mmap->PEN_2D_DIFF_RING_Y_ADDR, RawData_PenRingY_DiffMax, ts->x_gang_num * ts->y_num);
		nvt_read_get_num_mdata_36523(ts->mmap->PEN_2D_RAW_RING_Y_ADDR, RawData_PenRingY_DiffMin, ts->x_gang_num * ts->y_num);
	} /* if (ts->pen_support) */

	//---Leave Test Mode---
	nvt_change_mode_36523(NORMAL_MODE);
...
static int32_t nvt_read_fw_open(int32_t *xdata)
{
	...
	for (y = 0; y < IC_Y_CFG_SIZE; y++) {
		//---change xdata index---
		nvt_set_page_36523(raw_pipe_addr + y * IC_X_CFG_SIZE * 2);
		buf[0] = (uint8_t)((raw_pipe_addr + y * IC_X_CFG_SIZE * 2) & 0xFF);
		CTP_SPI_READ_36523(ts->client, buf, IC_X_CFG_SIZE * 2 + 1);
	...
	//---change xdata index---
	nvt_set_page_36523(raw_btn_pipe_addr);
	buf[0] = (uint8_t)(raw_btn_pipe_addr & 0xFF);
	CTP_SPI_READ_36523(ts->client, buf, IC_KEY_CFG_SIZE * 2 + 1);
...
static int32_t nvt_read_fw_short(int32_t *xdata)
{
	...
	for (y = 0; y < Y_Channel; y++) {
		//---change xdata index---
		nvt_set_page_36523(raw_pipe_addr + y * X_Channel * 2);
		buf[0] = (uint8_t)((raw_pipe_addr + y * X_Channel * 2) & 0xFF);
		CTP_SPI_READ_36523(ts->client, buf, X_Channel * 2 + 1);
	...
	nvt_set_page_36523(raw_btn_pipe_addr);
	buf[0] = (uint8_t)(raw_btn_pipe_addr & 0xFF);
	CTP_SPI_READ_36523(ts->client, buf, Key_Channel * 2 + 1);
...
static int32_t parse_mp_criteria_item_array(char **ptr, const char *item_string, int32_t *item_array,uint32_t x_num, uint32_t y_num)
{
	char *tmp = NULL;
	int32_t i = 0;
	int32_t j = 0;
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	if (nvt_get_dbgfw_status_36523()) {
		if (nvt_update_firmware_36523(DEFAULT_DEBUG_MP_NAME) < 0) {
			NVT_ERR("use built-in fw");
			nvt_update_firmware_36523(ts->mp_name);
		}
	} else {
		nvt_update_firmware_36523(ts->mp_name);
	}
	...
	if (nvt_load_mp_setting_criteria_from_csv(mp_setting_criteria_csv_filename) < 0) {
		...
		if(of_property_read_bool(np, "novatek,mp-support-dt")) {
			...
			if (nvt_mp_parse_dt_36523(np, mpcriteria)) {
				//---Download Normal FW---
	...
	if (nvt_check_fw_reset_state_36523(RESET_STATE_NORMAL_RUN)) {
		mutex_unlock(&ts->lock);
	...
	nvt_change_mode_36523(MP_MODE_CC);

	if (nvt_check_fw_status_36523()) {
		mutex_unlock(&ts->lock);
...
return:
	n.a.
*******************************************************/
int32_t nvt_mp_parse_dt_36523(struct device_node *root, const char *node_compatible)
{
...
*******************************************************/
int32_t nvt_mp_proc_init_36523(void)
{
...
return:
	n.a.
*******************************************************/
void nvt_mp_proc_deinit_36523(void)
{
	nvt_mp_buffer_deinit();
	if (NVT_proc_selftest_entry != NULL) {
...
#ifndef NVT_SAVE_TESTDATA_IN_FILE
static void test_buff_free(struct test_buf_36523 *buf)
{
	if(buf) {
...
static int32_t test_buff_init(struct test_buf_36523 **tbuf)
{
	struct test_buf_36523 *buf;
	int32_t ret;

	buf = (struct test_buf_36523 *)kzalloc(sizeof(*buf), GFP_KERNEL);
	if (!buf) {
...
static struct item_buf_36523 *item_arr[MAX_TEST_TYPE];
...
int32_t nvt_test_data_proc_init_36523(struct spi_device *client)
{
	int ret;
	struct test_buf_36523 *tbuf;
	struct nvt_ts_data_36523 *ts = spi_get_drvdata(client);
...
void nvt_test_data_proc_deinit_36523(void)
{

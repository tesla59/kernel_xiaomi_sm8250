...
static int32_t nvt_read_ram_and_save_file(uint32_t addr, uint16_t len, char *name)
{
	...
	nvt_set_page_36523(addr);

	fbufp[0] = addr & 0x7F;	//offset
	CTP_SPI_READ_36523(ts->client, fbufp, len+1);
		...
		uint32_t SRAM_addr, uint32_t size, uint32_t BIN_addr)
{
	...
	for (i = 0 ; i < count ; i++) {
		...
		ret = nvt_set_page_36523(SRAM_addr);
		if (ret) {
		...
		ret = CTP_SPI_WRITE_36523(ts->client, fwbuf, len+1);
		if (ret) {
...
static int32_t nvt_check_fw_checksum(void)
{
	...
	nvt_set_page_36523(ts->mmap->R_ILM_CHECKSUM_ADDR);

	/* read checksum */
	fwbuf[0] = (ts->mmap->R_ILM_CHECKSUM_ADDR) & 0x7F;
	ret = CTP_SPI_READ_36523(ts->client, fwbuf, len+1);
	if (ret) {
		...
		uint32_t G_CHECKSUM_ADDR, uint32_t crc)
{
	...
	nvt_set_page_36523(DES_ADDR);
	fwbuf[0] = DES_ADDR & 0x7F;
	fwbuf[1] = (SRAM_ADDR) & 0xFF;
	fwbuf[2] = (SRAM_ADDR >> 8) & 0xFF;
	fwbuf[3] = (SRAM_ADDR >> 16) & 0xFF;
	CTP_SPI_WRITE_36523(ts->client, fwbuf, 4);

	/* write length */
	//nvt_set_page_36523(LENGTH_ADDR);
	fwbuf[0] = LENGTH_ADDR & 0x7F;
	fwbuf[1] = (size) & 0xFF;
	fwbuf[2] = (size >> 8) & 0xFF;
	fwbuf[3] = (size >> 16) & 0x01;
	if (ts->hw_crc == 1) {
		CTP_SPI_WRITE_36523(ts->client, fwbuf, 3);
	} else if (ts->hw_crc > 1) {
		CTP_SPI_WRITE_36523(ts->client, fwbuf, 4);
	}

	/* write golden dlm checksum */
	//nvt_set_page_36523(G_CHECKSUM_ADDR);
	fwbuf[0] = G_CHECKSUM_ADDR & 0x7F;
	fwbuf[1] = (crc) & 0xFF;
	fwbuf[2] = (crc >> 8) & 0xFF;
	fwbuf[3] = (crc >> 16) & 0xFF;
	fwbuf[4] = (crc >> 24) & 0xFF;
	CTP_SPI_WRITE_36523(ts->client, fwbuf, 5);
...
static void nvt_read_bld_hw_crc(void)
{
	...
	/* CRC Flag */
	nvt_set_page_36523(ts->mmap->BLD_ILM_DLM_CRC_ADDR);
	...
	/* ILM CRC */
	nvt_set_page_36523(ts->mmap->G_ILM_CHECKSUM_ADDR);
	buf[0] = ts->mmap->G_ILM_CHECKSUM_ADDR & 0x7F;
	buf[1] = 0x00;
	buf[2] = 0x00;
	buf[3] = 0x00;
	buf[4] = 0x00;
	CTP_SPI_READ_36523(ts->client, buf, 5);
	...
	/* DLM CRC */
	nvt_set_page_36523(ts->mmap->G_DLM_CHECKSUM_ADDR);
	...
	nvt_set_page_36523(ts->mmap->R_DLM_CHECKSUM_ADDR);
...
static int32_t nvt_download_firmware_hw_crc(void)
{
	...
	while (1) {
		...
		if (cascade_2nd_header_info) {
			...
			ret = nvt_check_spi_dma_tx_info_36523();
			if (ret) {
		...
		nvt_bld_crc_enable_36523();

		/* clear fw reset status & enable fw crc check */
		nvt_fw_crc_enable_36523();

		/* Set Boot Ready Bit */
		nvt_boot_ready_36523();

		ret = nvt_check_fw_reset_state_36523(RESET_STATE_INIT);
		if (ret) {
...
static int32_t nvt_download_firmware(void)
{
	...
	while (1) {
		...
		nvt_write_addr_36523(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_RESET_COMPLETE, 0x00);

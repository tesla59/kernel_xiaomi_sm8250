...
struct nvt_ts_data_36523 *ts_36523;
...
#endif
static uint32_t ENG_RST_ADDR  = 0x7FFF80;
static uint32_t SWRST_N8_ADDR = 0; //read from dtsi
static uint32_t SPI_RD_FAST_ADDR = 0;	//read from dtsi
...
*******************************************************/
static inline int32_t spi_read_write(struct spi_device *client, uint8_t *buf, size_t len , NVT_SPI_RW_36523 rw)
{
...
*******************************************************/
int32_t CTP_SPI_READ_36523(struct spi_device *client, uint8_t *buf, uint16_t len)
{
	int32_t ret = -1;
	int32_t retries = 0;
...
*******************************************************/
int32_t CTP_SPI_WRITE_36523(struct spi_device *client, uint8_t *buf, uint16_t len)
{
	int32_t ret = -1;
	int32_t retries = 0;
...
*******************************************************/
int32_t nvt_set_page_36523(uint32_t addr)
{
	uint8_t buf[4] = {0};
	...
	return CTP_SPI_WRITE_36523(ts->client, buf, 3);
}
...
return:
	Executive outcomes. 0---succeed. -5---access fail.
*******************************************************/
int32_t nvt_write_addr_36523(uint32_t addr, uint8_t data)
{
	int32_t ret = 0;
	uint8_t buf[4] = {0};
	...
	buf[1] = (addr >> 15) & 0xFF;
	buf[2] = (addr >> 7) & 0xFF;
	ret = CTP_SPI_WRITE_36523(ts->client, buf, 3);
	if (ret) {
	...
	buf[0] = addr & (0x7F);
	buf[1] = data;
	ret = CTP_SPI_WRITE_36523(ts->client, buf, 2);
	if (ret) {
...
return:
	N/A.
*******************************************************/
void nvt_bld_crc_enable_36523(void)
{
	uint8_t buf[4] = {0};

	//---set xdata index to BLD_CRC_EN_ADDR---
	nvt_set_page_36523(ts->mmap->BLD_CRC_EN_ADDR);

	//---read data from index---
	buf[0] = ts->mmap->BLD_CRC_EN_ADDR & (0x7F);
	buf[1] = 0xFF;
	CTP_SPI_READ_36523(ts->client, buf, 2);

	//---write data to index---
	buf[0] = ts->mmap->BLD_CRC_EN_ADDR & (0x7F);
	buf[1] = buf[1] | (0x01 << 7);
	CTP_SPI_WRITE_36523(ts->client, buf, 2);
}
...
return:
	N/A.
*******************************************************/
void nvt_fw_crc_enable_36523(void)
{
	uint8_t buf[4] = {0};

	//---set xdata index to EVENT BUF ADDR---
	nvt_set_page_36523(ts->mmap->EVENT_BUF_ADDR);
...
return:
	N/A.
*******************************************************/
void nvt_boot_ready_36523(void)
{
	//---write BOOT_RDY status cmds---
	nvt_write_addr_36523(ts->mmap->BOOT_RDY_ADDR, 1);

	mdelay(5);

	if (!ts->hw_crc) {
		//---write BOOT_RDY status cmds---
		nvt_write_addr_36523(ts->mmap->BOOT_RDY_ADDR, 0);

		//---write POR_CD cmds---
		nvt_write_addr_36523(ts->mmap->POR_CD_ADDR, 0xA0);
	}
}
...
return:
	N/A.
*******************************************************/
void nvt_tx_auto_copy_mode_36523(void)
{
	//---write TX_AUTO_COPY_EN cmds---
	nvt_write_addr_36523(ts->mmap->TX_AUTO_COPY_EN, 0x69);
...
return:
	N/A.
*******************************************************/
int32_t nvt_check_spi_dma_tx_info_36523(void)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
	...
	for (i = 0; i < retry; i++) {
		//---set xdata index to EVENT BUF ADDR---
		nvt_set_page_36523(ts->mmap->SPI_DMA_TX_INFO);
...
return:
	n.a.
*******************************************************/
void nvt_eng_reset_36523(void)
{
	//---eng reset cmds to ENG_RST_ADDR---
	nvt_write_addr_36523(ENG_RST_ADDR, 0x5A);
...
return:
	n.a.
*******************************************************/
void nvt_sw_reset_36523(void)
{
	//---software reset cmds to SWRST_N8_ADDR---
	nvt_write_addr_36523(SWRST_N8_ADDR, 0x55);
...
return:
	n.a.
*******************************************************/
void nvt_sw_reset_idle_36523(void)
{
	//---MCU idle cmds to SWRST_N8_ADDR---
	nvt_write_addr_36523(SWRST_N8_ADDR, 0xAA);
...
return:
	n.a.
*******************************************************/
void nvt_bootloader_reset_36523(void)
{
	//---reset cmds to SWRST_N8_ADDR---
	nvt_write_addr_36523(SWRST_N8_ADDR, 0x69);

	mdelay(5);	//wait tBRST2FR after Bootload RST

	if (SPI_RD_FAST_ADDR) {
		/* disable SPI_RD_FAST */
		nvt_write_addr_36523(SPI_RD_FAST_ADDR, 0x00);
	}
}
...
*******************************************************/
int32_t nvt_clear_fw_status_36523(void)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
	...
	for (i = 0; i < retry; i++) {
		//---set xdata index to EVENT BUF ADDR---
		nvt_set_page_36523(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_HANDSHAKING_or_SUB_CMD_BYTE);
...
*******************************************************/
int32_t nvt_check_fw_status_36523(void)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
...
return:
	Executive outcomes. 0---succeed. -1---failed.
*******************************************************/
int32_t nvt_check_fw_reset_state_36523(RST_COMPLETE_STATE_36523 check_reset_state)
{
	uint8_t buf[8] = {0};
	int32_t ret = 0;
	int32_t retry = 0;
	...
	//---set xdata index to EVENT BUF ADDR---
	nvt_set_page_36523(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_RESET_COMPLETE);

	while (1) {
		//---read reset state---
		buf[0] = EVENT_MAP_RESET_COMPLETE;
		buf[1] = 0x00;
		CTP_SPI_READ_36523(ts->client, buf, 6);
...
static int32_t nvt_read_pid(void)
{
	...
	//---set xdata index to EVENT BUF ADDR---
	nvt_set_page_36523(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_PROJECTID);

	//---read project id---
	buf[0] = EVENT_MAP_PROJECTID;
	buf[1] = 0x00;
	buf[2] = 0x00;
	CTP_SPI_READ_36523(ts->client, buf, 3);
...
return:
	Executive outcomes. 0---success. -1---fail.
*******************************************************/
int32_t nvt_get_fw_info_36523(void)
{
	uint8_t buf[64] = {0};
...
info_retry:
	//---set xdata index to EVENT BUF ADDR---
	nvt_set_page_36523(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_FWINFO);

	//---read fw info---
	buf[0] = EVENT_MAP_FWINFO;
	CTP_SPI_READ_36523(ts->client, buf, 39);
	ts->fw_ver = buf[1];
	ts->x_num = buf[3];
	ts->y_num = buf[4];
...
static ssize_t nvt_flash_read(struct file *file, char __user *buff, size_t count, loff_t *offp)
{
	...
	nvt_esd_check_enable_36523(false);
#endif /* #if NVT_TOUCH_ESD_PROTECT */
	...
	if (spi_wr == NVTWRITE) {	//SPI write
		while (retries < 20) {
			ret = CTP_SPI_WRITE_36523(ts->client, buf, ((str[0] & 0x7F) << 8) | str[1]);
			if (!ret)
				break;
			else
		...
		while (retries < 20) {
			ret = CTP_SPI_READ_36523(ts->client, buf, ((str[0] & 0x7F) << 8) | str[1]);
			if (!ret)
				break;
			else
				NVT_ERR("error, retries=%d, ret=%d\n", retries, ret);
...
{
	struct nvt_flash_data_36523 *dev;

	dev = kmalloc(sizeof(struct nvt_flash_data_36523), GFP_KERNEL);
	if (dev == NULL) {
...
{
	struct nvt_flash_data_36523 *dev = file->private_data;
...
{
	struct nvt_config_info_36523 *config_info;
	...
	ts->config_array = devm_kzalloc(dev, ts->config_array_size * sizeof(struct nvt_config_info_36523), GFP_KERNEL);
	if (!ts->config_array) {
...
static int nvt_get_panel_type(struct nvt_ts_data_36523 *ts_data)
{
	int i;
	int j;
	u8 *lockdown = ts_data->lockdown_info;
	struct nvt_config_info_36523 *panel_list = ts->config_array;
...
static bool is_lockdown_empty(u8 *lockdown)
{
	bool ret = true;
	int i;
...
*******************************************************/
static int nvt_gpio_config(struct nvt_ts_data_36523 *ts)
{
	int32_t ret = 0;
...
return:
	n.a.
*******************************************************/
static void nvt_gpio_deconfig(struct nvt_ts_data_36523 *ts)
{
	if (gpio_is_valid(ts->irq_gpio))
...
void nvt_set_dbgfw_status_36523(bool enable)
{
	ts->fw_debug = enable;
}

bool nvt_get_dbgfw_status_36523(void)
{
	return ts->fw_debug;
}
...
#if NVT_TOUCH_ESD_PROTECT
void nvt_esd_check_enable_36523(uint8_t enable)
{
...
static void nvt_esd_check_func(struct work_struct *work)
{
	...
	if ((timer > NVT_TOUCH_ESD_CHECK_PERIOD) && esd_check) {
		...
		if (nvt_get_dbgfw_status_36523()) {
			if (nvt_update_firmware_36523(DEFAULT_DEBUG_FW_NAME) < 0) {
				NVT_ERR("use built-in fw");
				nvt_update_firmware_36523(ts->fw_name);
			}
		} else {
			nvt_update_firmware_36523(ts->fw_name);
		}
...
static uint32_t nvt_dump_fw_history(void)
{
	...
	nvt_set_page_36523(ts->mmap->FW_HISTORY_ADDR);
	buf[0] = ts->mmap->FW_HISTORY_ADDR & 0xFF;
	CTP_SPI_READ_36523(ts->client, buf, FW_HISTORY_SIZE + 1);
	if (ret) {
...
static irqreturn_t nvt_ts_work_func(int irq, void *data)
{
	...
	if (ts->pen_support)
		ret = CTP_SPI_READ_36523(ts->client, point_data, POINT_DATA_LEN + PEN_DATA_LEN + 1);
	else
		ret = CTP_SPI_READ_36523(ts->client, point_data, POINT_DATA_LEN + 1);
	if (ret < 0) {
		NVT_ERR("CTP_SPI_READ failed.(%d)\n", ret);
		goto XFER_ERROR;
	}
...
#if NVT_TOUCH_ESD_PROTECT
		nvt_esd_check_enable_36523(true);
#endif /* #if NVT_TOUCH_ESD_PROTECT */
		goto XFER_ERROR;
	}
...
static int8_t nvt_ts_check_chip_ver_trim(uint32_t chip_ver_trim_addr)
{
	...
	for (retry = 5; retry > 0; retry--) {
		...
		nvt_bootloader_reset_36523();

		nvt_set_page_36523(chip_ver_trim_addr);
		...
		buf[5] = 0x00;
		buf[6] = 0x00;
		CTP_SPI_READ_36523(ts->client, buf, 7);
...
static int nvt_touchfeature_cmd_xsfer(uint8_t *touchfeature)
{
	...
	/* ---set xdata index to EVENT BUF ADDR--- */
	ret = nvt_set_page_36523(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_HOST_CMD);
	if (ret < 0) {
...
{
	struct nvt_ts_data_36523 *ts =
	    container_of(nb, struct nvt_ts_data_36523, power_supply_notifier);
...
{
	struct nvt_ts_data_36523 *ts =
		container_of(work, struct nvt_ts_data_36523, power_supply_work);
	int is_usb_exist = 0;
	uint8_t buf[3] = {0};
	int32_t ret;
...
static void tpdbg_shutdown(struct nvt_ts_data_36523 *ts_core, bool enable)
{
	mutex_lock(&ts->lock);
	if (enable) {
		if (nvt_write_addr_36523(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_HOST_CMD, 0x1A) < 0) {
			NVT_ERR("disable tp sensor failed!");
		}
	} else {
		if (nvt_write_addr_36523(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_HOST_CMD, 0x15) < 0) {
...
static void tpdbg_suspend(struct nvt_ts_data_36523 *ts_core, bool enable)
{
	if (enable)
...
{
	struct nvt_ts_data_36523 *ts_core = file->private_data;
	...
	if (!strncmp(cmd, "irq-disable", 11))
		...
		nvt_set_dbgfw_status_36523(true);
	else if (!strncmp(cmd, "fw-debug-off", 12))
		nvt_set_dbgfw_status_36523(false);
out:
	kfree(cmd);
...
{
	struct nvt_ts_data_36523 *ts_core = container_of(work, struct nvt_ts_data_36523, resume_work);
	nvt_ts_resume(&ts_core->client->dev);
}

static int nvt_pinctrl_init(struct nvt_ts_data_36523 *nvt_data)
{
	int retval = 0;
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	ts = kzalloc(sizeof(struct nvt_ts_data_36523), GFP_KERNEL);
	if (ts == NULL) {
	...
	INIT_DELAYED_WORK(&ts->nvt_fwu_work, Boot_Update_Firmware_36523);
	// please make sure boot update start after display reset(RESX) sequence
...
#ifndef NVT_SAVE_TESTDATA_IN_FILE
	ret = nvt_test_data_proc_init_36523(ts->client);
	if (ret < 0) {
...
#ifndef NVT_SAVE_TESTDATA_IN_FILE
	nvt_test_data_proc_deinit_36523();
#endif
#if NVT_TOUCH_MP
	nvt_mp_proc_deinit_36523();
err_mp_proc_init_failed:
#endif
#if NVT_TOUCH_EXT_PROC
	nvt_extra_proc_deinit_36523();
...
static int32_t nvt_ts_resume(struct device *dev)
{
...
#endif
	if (nvt_get_dbgfw_status_36523()) {
		ret = nvt_update_firmware_36523(DEFAULT_DEBUG_FW_NAME);
		if (ret < 0) {
			NVT_ERR("use built-in fw");
			ret = nvt_update_firmware_36523(ts->fw_name);
		}
	} else {
		ret = nvt_update_firmware_36523(ts->fw_name);
	}
	...
	nvt_check_fw_reset_state_36523(RESET_STATE_REK);
...
static int nvt_drm_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	int *blank;
	struct nvt_ts_data_36523 *ts =
		container_of(self, struct nvt_ts_data_36523, drm_notif);
...
static int nvt_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	int *blank;
	struct nvt_ts_data_36523 *ts =
		container_of(self, struct nvt_ts_data_36523, fb_notif);
...
static struct of_device_id nvt_match_table[] = {
	{ .compatible = "novatek,NVT-ts-spi-36523",},
	{ },
};
#endif

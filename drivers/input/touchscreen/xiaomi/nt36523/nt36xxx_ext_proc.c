...
return:
	n.a.
*******************************************************/
void nvt_change_mode_36523(uint8_t mode)
{
	uint8_t buf[8] = {0};
...
int32_t nvt_set_pen_normal_mode(void)
{
	...
	buf[1] = 0xC1;
	buf[2] = 0x04;
	CTP_SPI_WRITE_36523(ts->client, buf, 3);
...
*******************************************************/
uint8_t nvt_get_fw_pipe_36523(void)
{
	uint8_t buf[8]= {0};
...
return:
	n.a.
*******************************************************/
void nvt_read_mdata_36523(uint32_t xdata_addr, uint32_t xdata_btn_addr)
{
	int32_t i = 0;
	int32_t j = 0;
	int32_t k = 0;
	...
	if (residual_len != 0) {
		//---change xdata index---
		nvt_set_page_36523(xdata_addr + data_len - residual_len);
	...
	//---change xdata index---
	nvt_set_page_36523(xdata_btn_addr);

	//---read data---
	buf[0] = (xdata_btn_addr & 0xFF);
	CTP_SPI_READ_36523(ts->client, buf, (TOUCH_KEY_NUM * 2 + 1));
...
return:
    n.a.
*******************************************************/
void nvt_get_mdata_36523(int32_t *buf, uint8_t *m_x_num, uint8_t *m_y_num)
{
	*m_x_num = ts->x_num;
	*m_y_num = ts->y_num;
...
return:
	n.a.
*******************************************************/
void nvt_read_get_num_mdata_36523(uint32_t xdata_addr, int32_t *buffer, uint32_t num)
{
	int32_t i = 0;
	int32_t j = 0;
	int32_t k = 0;
...
static const struct seq_operations nvt_pen_diff_seq_ops = {
	.start  = c_start,
	.next   = c_next,
	.stop   = c_stop,
...
static int32_t nvt_baseline_open(struct inode *inode, struct file *file)
{
	...
	nvt_change_mode_36523(TEST_MODE_2);
...
static int32_t nvt_raw_open(struct inode *inode, struct file *file)
{
	...
	if (nvt_get_fw_pipe_36523() == 0)
		nvt_read_mdata_36523(ts->mmap->RAW_PIPE0_ADDR, ts->mmap->RAW_BTN_PIPE0_ADDR);
	else
		nvt_read_mdata_36523(ts->mmap->RAW_PIPE1_ADDR, ts->mmap->RAW_BTN_PIPE1_ADDR);
...
static int32_t nvt_pen_diff_open(struct inode *inode, struct file *file)
{
	...
	nvt_read_get_num_mdata_36523(ts->mmap->PEN_1D_DIFF_TIP_X_ADDR, xdata_pen_tip_x, ts->x_num);
	nvt_read_get_num_mdata_36523(ts->mmap->PEN_1D_DIFF_TIP_Y_ADDR, xdata_pen_tip_y, ts->y_num);
	nvt_read_get_num_mdata_36523(ts->mmap->PEN_1D_DIFF_RING_X_ADDR, xdata_pen_ring_x, ts->x_num);
	nvt_read_get_num_mdata_36523(ts->mmap->PEN_1D_DIFF_RING_Y_ADDR, xdata_pen_ring_y, ts->y_num);

	nvt_change_mode_36523(NORMAL_MODE);

	nvt_set_pen_normal_mode();

	nvt_check_fw_reset_state_36523(RESET_STATE_NORMAL_RUN);
...
int32_t nvt_get_pf_switch(uint8_t *pf_switch)
{
	...
	msleep(35);
	//---set xdata index to EVENT BUF ADDR---
	ret = nvt_set_page_36523(ts->mmap->EVENT_BUF_ADDR | 0x5D);
	if (ret < 0) {
		NVT_ERR("Set event buffer index fail!\n");
	...
	buf[0] = 0x5D;
	buf[1] = 0x00;
	ret = CTP_SPI_READ_36523(ts->client, buf, 2);
	if (ret < 0) {
...
int32_t nvt_get_edge_reject_switch(uint8_t *edge_reject_switch)
{
	...
	//---set xdata index to EVENT BUF ADDR---
	ret = nvt_set_page_36523(ts->mmap->EVENT_BUF_ADDR | 0x5C);
	if (ret < 0) {
		NVT_ERR("Set event buffer index fail!\n");
...
int32_t nvt_get_hand_only_switch(uint8_t *hand_only_switch)
{
	...
	//---set xdata index to EVENT BUF ADDR---
	ret = nvt_set_page_36523(ts->mmap->EVENT_BUF_ADDR | 0x5E);
	if (ret < 0) {
		NVT_ERR("Set event buffer index fail!\n");
...
int32_t nvt_get_small_jitter_switch(uint8_t *small_jitter_switch)
{
	...
	buf[1] = 0x00;
	buf[2] = 0x00;
	ret = CTP_SPI_READ_36523(ts->client, buf, 3);
	if (ret < 0) {
...
int32_t nvt_get_game_mode_switch(uint8_t *game_mode_switch)
{
	...
	//---set xdata index to EVENT BUF ADDR---
	ret = nvt_set_page_36523(ts->mmap->EVENT_BUF_ADDR | 0x5F);
	if (ret < 0) {
		NVT_ERR("Set event buffer index fail!\n");
...
*******************************************************/
int32_t nvt_extra_proc_init_36523(void)
{
...
return:
	n.a.
*******************************************************/
void nvt_extra_proc_deinit_36523(void)
{
	if (NVT_proc_fw_version_entry != NULL) {

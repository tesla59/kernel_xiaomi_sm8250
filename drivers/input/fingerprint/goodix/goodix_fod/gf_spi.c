...
#define GF_SPIDEV_NAME			"goodix,fingerprint_fod"
/*device name after register in charater*/
#define GF_DEV_NAME				"gx_fod_fp"
#define GF_INPUT_NAME			"uinput-gx_fod"	/*"goodix_fp" */

#define CHRD_DRIVER_NAME		"goodix_fp_spi"
#define CLASS_NAME				"gx_fod_fp"
...
#if defined(USE_SPI_BUS)
static int gf_fod_probe(struct spi_device *spi)
#elif defined(USE_PLATFORM_BUS)
static int gf_fod_probe(struct platform_device *pdev)
#endif
{
	struct gf_dev *gf_dev = &gf;
	int status = -EINVAL;
...
#endif
	gf_dev->irq = gf_fod_irq_num(gf_dev);
...
error_hw:
	gf_fod_cleanup(gf_dev);
	gf_dev->device_available = 0;
	return status;
}

#if defined(USE_SPI_BUS)
static int gf_fod_remove(struct spi_device *spi)
#elif defined(USE_PLATFORM_BUS)
static int gf_fod_remove(struct platform_device *pdev)
#endif
{
	struct gf_dev *gf_dev = &gf;
...
#if defined(USE_SPI_BUS)
static struct spi_driver gf_fod_driver = {
#elif defined(USE_PLATFORM_BUS)
static struct platform_driver gf_fod_driver = {
#endif
	.driver = {
		.name = GF_DEV_NAME,
		.owner = THIS_MODULE,
	...
	.probe = gf_fod_probe,
	.remove = gf_fod_remove,
};

static int __init gf_fod_init(void)
{
	int status;
	...
	if (IS_ERR(gf_class)) {
		unregister_chrdev(SPIDEV_MAJOR, gf_fod_driver.driver.name);
		pr_warn("Failed to create class.\n");
		return PTR_ERR(gf_class);
	}
#if defined(USE_PLATFORM_BUS)
	status = platform_driver_register(&gf_fod_driver);
#elif defined(USE_SPI_BUS)
	status = spi_register_driver(&gf_fod_driver);
#endif
...
module_init(gf_fod_init);

static void __exit gf_fod_exit(void)
{
#ifdef GF_NETLINK_ENABLE
	netlink_exit();
#endif
#if defined(USE_PLATFORM_BUS)
	platform_driver_unregister(&gf_fod_driver);
#elif defined(USE_SPI_BUS)
	spi_unregister_driver(&gf_fod_driver);
#endif
	class_destroy(gf_class);
	unregister_chrdev(SPIDEV_MAJOR, gf_fod_driver.driver.name);
}
module_exit(gf_fod_exit);

...
static void fpc_hotzone_finger_irq_set(uint8_t flag)
{
	finger_irq_value = flag;
}

static uint8_t fpc_hotzone_finger_irq_get(void)
{
...
static int fpc1020_fod_probe(struct platform_device *pdev)
{
...
static int fpc1020_fod_remove(struct platform_device *pdev)
{
...
static struct of_device_id fpc1020_fod_of_match[] = {
	{ .compatible = "fpc,fpc1020_fod", },
	{}
};
MODULE_DEVICE_TABLE(of, fpc1020_fod_of_match);

static struct platform_driver fpc1020_fod_driver = {
	.driver = {
		.name	= "fpc1020_fod",
		.owner	= THIS_MODULE,
		.of_match_table = fpc1020_fod_of_match,
	},
	.probe	= fpc1020_fod_probe,
	.remove	= fpc1020_fod_remove,
};

static int __init fpc1020_fod_init(void)
{
	int rc = platform_driver_register(&fpc1020_fod_driver);
...
static void __exit fpc1020_fod_exit(void)
{
	pr_info("%s\n", __func__);
	platform_driver_unregister(&fpc1020_fod_driver);
}

module_init(fpc1020_fod_init);
module_exit(fpc1020_fod_exit);

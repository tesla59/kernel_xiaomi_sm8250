...
#define AUDIO_SMARTPA_STEREO \
	(socinfo_get_platform_type() == HW_PLATFORM_ALIOTH || \
	 ...
	 socinfo_get_platform_type() == HW_PLATFORM_ELISH || \
...
#define AUDIO_SMARTPA_STEREO \
	(socinfo_get_platform_type() == HW_PLATFORM_MONET || \
	 socinfo_get_platform_type() == HW_PLATFORM_VANGOGH)
#else
#define AUDIO_SMARTPA_STEREO (0)
#endif
...
static int wm_adsp_load(struct wm_adsp *dsp)
{
	...
	case WMFW_HALO:
		if (dsp->firmwares[dsp->fw].fullname)
			...
			if (AUDIO_SMARTPA_STEREO && dsp->chip_revid == 0xB2) {
...
#ifdef CONFIG_MACH_XIAOMI_SM8250
				if (socinfo_get_platform_type() == HW_PLATFORM_ALIOTH ||
				    socinfo_get_platform_type() == HW_PLATFORM_APOLLO ||
					socinfo_get_platform_type() == HW_PLATFORM_CAS) {
					//for ultrasonic

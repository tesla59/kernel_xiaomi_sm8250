...
#define NXP_SPEAKERBOOST_V3 \
	(socinfo_get_platform_type() == HW_PLATFORM_LMI)
#else
#define NXP_SPEAKERBOOST_V3 (0)
#endif

#if defined(CONFIG_MACH_XIAOMI_SM8250)
#define TFA_NON_DSP_SOLUTION \
	(socinfo_get_platform_type() == HW_PLATFORM_LMI)
#elif defined(CONFIG_MACH_XIAOMI_SM7250)
#define TFA_NON_DSP_SOLUTION \
	(socinfo_get_platform_type() == HW_PLATFORM_PICASSO)
#else
#define TFA_NON_DSP_SOLUTION (0)
#endif
...
static int tfa98xx_read_memtrack_data(struct tfa98xx *tfa98xx, int *pLivedata)
{
	...
	struct livedata_cfg livedata_table_v2[TFA_DEVICE_TYPE_MAX][MEMTRACK_ITEM_MAX] = {
		/* for TFA9894 device */
		{
	...
	struct livedata_cfg livedata_table_v3[TFA_DEVICE_TYPE_MAX][MEMTRACK_ITEM_MAX] = {
		/* for TFA9894 device */
		{
			{0x6fc, 0x02, 0x0001},  /* fRes_P */
			{0x705, 0x02, 0x4000},  /* T_P */
	...
	struct livedata_cfg (*livedata_table)[TFA_DEVICE_TYPE_MAX][MEMTRACK_ITEM_MAX] =
		(NXP_SPEAKERBOOST_V3) ? &livedata_table_v3 : &livedata_table_v2;
	...
	pr_info("Read livedata from Speakerboost %s\n", (NXP_SPEAKERBOOST_V3 ? "3.0" : "2.0"));

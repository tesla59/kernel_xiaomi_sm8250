...
static struct wm_adsp_ops_v2 wm_adsp1_ops;
static struct wm_adsp_ops_v2 wm_adsp2_ops[];
static struct wm_adsp_ops_v2 wm_halo_ops;
static struct wm_adsp_ops_v2 wm_vpu_ops;
...
struct wm_adsp_compr_buf_v2 {
	struct list_head list;
	struct wm_adsp_v2 *dsp;
	struct wm_adsp_compr_v2 *compr;
...
struct wm_adsp_compr_v2 {
	struct list_head list;
	struct wm_adsp_v2 *dsp;
	struct wm_adsp_compr_buf_v2 *buf;
...
static int wm_adsp_buffer_init(struct wm_adsp_v2 *dsp);
static int wm_adsp_buffer_free(struct wm_adsp_v2 *dsp);
...
struct wm_coeff_ctl {
	...
	struct wm_adsp_alg_region_v2 alg_region;
...
static void wm_adsp_debugfs_save_wmfwname(struct wm_adsp_v2 *dsp, const char *s)
{
...
static void wm_adsp_debugfs_save_binname(struct wm_adsp_v2 *dsp, const char *s)
{
	char *tmp = kasprintf(GFP_KERNEL, "%s\n", s);
...
static void wm_adsp_debugfs_clear(struct wm_adsp_v2 *dsp)
{
	kfree(dsp->wmfw_file_name);
	kfree(dsp->bin_file_name);
...
{
	struct wm_adsp_v2 *dsp = file->private_data;
	ssize_t ret;
...
static void wm_adsp2_init_debugfs(struct wm_adsp_v2 *dsp,
...
static void wm_adsp2_cleanup_debugfs(struct wm_adsp_v2 *dsp)
{
	wm_adsp_debugfs_clear(dsp);
	debugfs_remove_recursive(dsp->debugfs_root);
}
#else
static inline void wm_adsp2_init_debugfs(struct wm_adsp_v2 *dsp,
					 struct snd_soc_component *component)
{
}

static inline void wm_adsp2_cleanup_debugfs(struct wm_adsp_v2 *dsp)
{
}

static inline void wm_adsp_debugfs_save_wmfwname(struct wm_adsp_v2 *dsp,
						 const char *s)
{
}

static inline void wm_adsp_debugfs_save_binname(struct wm_adsp_v2 *dsp,
						const char *s)
{
}

static inline void wm_adsp_debugfs_clear(struct wm_adsp_v2 *dsp)
{
}
#endif
		   ...
		   struct snd_ctl_elem_value *ucontrol)
{
	...
	struct wm_adsp_v2 *dsp = snd_soc_component_get_drvdata(component);

	ucontrol->value.enumerated.item[0] = dsp[e->shift_l].fw;

	return 0;
}
EXPORT_SYMBOL_GPL(wm_adsp_fw_get_v2);
...
EXPORT_SYMBOL_GPL(wm_adsp_fw_put_v2);
...
static struct wm_adsp_region_v2 const *wm_adsp_find_region(struct wm_adsp_v2 *dsp,
							int type)
{
	int i;
...
static unsigned int wm_adsp_region_to_reg(struct wm_adsp_region_v2 const *mem,
					  unsigned int offset)
{
	switch (mem->type) {
	case WMFW_ADSP1_PM:
...
static unsigned int wm_halo_region_to_reg(struct wm_adsp_region_v2 const *mem,
					  unsigned int offset)
{
	switch (mem->type) {
	case WMFW_ADSP2_XM:
...
static unsigned int wm_vpu_region_to_reg(struct wm_adsp_region_v2 const *mem,
					 unsigned int offset)
{
	switch (mem->type) {
	case WMFW_VPU_DM:
...
static void wm_adsp_read_fw_status(struct wm_adsp_v2 *dsp,
...
static void wm_adsp2_show_fw_status(struct wm_adsp_v2 *dsp)
{
	unsigned int offs[] = {
...
static void wm_adsp2v2_show_fw_status(struct wm_adsp_v2 *dsp)
{
...
static void wm_halo_show_fw_status(struct wm_adsp_v2 *dsp)
{
	unsigned int offs[] = {
...
{
	const struct wm_adsp_alg_region_v2 *alg_region = &ctl->alg_region;
	struct wm_adsp_v2 *dsp = ctl->dsp;
...
static int wm_coeff_read_control_v2(struct wm_coeff_ctl *ctl,
				 void *buf, size_t len)
{
	struct wm_adsp_v2 *dsp = ctl->dsp;
	void *scratch;
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ctl->flags & WMFW_CTL_FLAG_VOLATILE) {
		if (ctl->enabled && ctl->dsp->running)
			ret = wm_coeff_read_control_v2(ctl, p, ctl->len);
		else
			ret = -EPERM;
	} else {
		if (!ctl->flags && ctl->enabled && ctl->dsp->running)
			ret = wm_coeff_read_control_v2(ctl, ctl->cache, ctl->len);
			    ...
			    unsigned int __user *bytes, unsigned int size)
{
	...
	if (ctl->flags & WMFW_CTL_FLAG_VOLATILE) {
		if (ctl->enabled && ctl->dsp->running)
			ret = wm_coeff_read_control_v2(ctl, ctl->cache, size);
		else
			ret = -EPERM;
	} else {
		if (!ctl->flags && ctl->enabled && ctl->dsp->running)
...
static int wmfw_add_ctl(struct wm_adsp_v2 *dsp, struct wm_coeff_ctl *ctl)
{
...
static int wm_coeff_init_control_caches(struct wm_adsp_v2 *dsp)
{
	struct wm_coeff_ctl *ctl;
	int ret;
...
static int wm_coeff_sync_controls(struct wm_adsp_v2 *dsp)
{
	struct wm_coeff_ctl *ctl;
	int ret;
...
static void wm_adsp_signal_event_controls(struct wm_adsp_v2 *dsp,
					  unsigned int event)
{
	struct wm_coeff_ctl *ctl;
	int ret;
...
static int wm_adsp_create_control(struct wm_adsp_v2 *dsp,
				  const struct wm_adsp_alg_region_v2 *alg_region,
...
static inline void wm_coeff_parse_alg(struct wm_adsp_v2 *dsp, const u8 **data,
...
static inline void wm_coeff_parse_coeff(struct wm_adsp_v2 *dsp, const u8 **data,
...
static int wm_adsp_check_coeff_flags(struct wm_adsp_v2 *dsp,
...
static int wm_adsp_parse_coeff(struct wm_adsp_v2 *dsp,
...
static int wm_adsp_write_blocks(struct wm_adsp_v2 *dsp, const u8 *data, size_t len,
...
static unsigned int wm_adsp1_parse_sizes(struct wm_adsp_v2 *dsp,
...
static unsigned int wm_adsp2_parse_sizes(struct wm_adsp_v2 *dsp,
					 const char * const file,
					 unsigned int pos,
					 const struct firmware *firmware)
{
...
static unsigned int wm_vpu_parse_sizes(struct wm_adsp_v2 *dsp,
				       const char * const file,
				       unsigned int pos,
				       const struct firmware *firmware)
{
...
static bool wm_adsp_validate_version(struct wm_adsp_v2 *dsp, unsigned int version)
{
	switch (version) {
	case 0:
...
static bool wm_halo_validate_version(struct wm_adsp_v2 *dsp, unsigned int version)
{
	switch (version) {
	case 3:
		return true;
...
static int wm_adsp_load(struct wm_adsp_v2 *dsp)
{
	LIST_HEAD(buf_list);
	const struct firmware *firmware;
 ...
 */
static struct wm_coeff_ctl *wm_adsp_get_ctl(struct wm_adsp_v2 *dsp,
					     const char *name)
{
...
EXPORT_SYMBOL_GPL(wm_adsp_write_ctl_v2);
		     ...
		     size_t len)
{
	...
	return wm_coeff_read_control_v2(ctl, buf, len);
}
EXPORT_SYMBOL_GPL(wm_adsp_read_ctl_v2);

static void wm_adsp_ctl_fixup_base(struct wm_adsp_v2 *dsp,
				  const struct wm_adsp_alg_region_v2 *alg_region)
{
	struct wm_coeff_ctl *ctl;
...
static void *wm_adsp_read_algs(struct wm_adsp_v2 *dsp, size_t n_algs,
			       const struct wm_adsp_region_v2 *mem,
...
static struct wm_adsp_alg_region_v2 *
	wm_adsp_find_alg_region(struct wm_adsp_v2 *dsp, int type, unsigned int id)
{
	struct wm_adsp_alg_region_v2 *alg_region;
...
static struct wm_adsp_alg_region_v2 *wm_adsp_create_region(struct wm_adsp_v2 *dsp,
							int type, __be32 id,
							__be32 base)
{
	struct wm_adsp_alg_region_v2 *alg_region;
...
static void wm_adsp_free_alg_regions(struct wm_adsp_v2 *dsp)
{
	struct wm_adsp_alg_region_v2 *alg_region;
...
static void wmfw_parse_id_header(struct wm_adsp_v2 *dsp,
...
static void wmfw_v3_parse_id_header(struct wm_adsp_v2 *dsp,
...
static int wm_adsp_create_regions(struct wm_adsp_v2 *dsp, __be32 id, int nregions,
...
static int wm_adsp1_setup_algs(struct wm_adsp_v2 *dsp)
{
...
static int wm_adsp2_setup_algs(struct wm_adsp_v2 *dsp)
{
...
static int wm_halo_create_regions(struct wm_adsp_v2 *dsp, __be32 id,
...
static int wm_halo_setup_algs(struct wm_adsp_v2 *dsp)
{
...
static int wm_vpu_setup_algs(struct wm_adsp_v2 *vpu)
{
	const struct wm_adsp_region_v2 *mem;
...
static int wm_adsp_load_coeff(struct wm_adsp_v2 *dsp)
{
	LIST_HEAD(buf_list);
	struct regmap *regmap = dsp->regmap;
...
static int wm_adsp_create_name(struct wm_adsp_v2 *dsp)
{
	char *p;
...
static int wm_adsp_common_init(struct wm_adsp_v2 *dsp)
{
	int ret;
...
EXPORT_SYMBOL_GPL(wm_adsp1_init_v2);
		   ...
		   int event)
{
	...
	struct wm_adsp_v2 *dsps = snd_soc_component_get_drvdata(component);
	struct wm_adsp_v2 *dsp = &dsps[w->shift];
	struct wm_coeff_ctl *ctl;
	int ret;
	unsigned int val;
...
EXPORT_SYMBOL_GPL(wm_adsp1_event_v2);

static int wm_adsp2v2_enable_core(struct wm_adsp_v2 *dsp)
{
	unsigned int val;
	int ret, count;
...
static int wm_adsp2_enable_core(struct wm_adsp_v2 *dsp)
{
	int ret;
...
static int wm_adsp2_lock(struct wm_adsp_v2 *dsp, unsigned int lock_regions)
{
	struct regmap *regmap = dsp->regmap;
...
static int wm_adsp2_enable_memory(struct wm_adsp_v2 *dsp)
{
	return regmap_update_bits(dsp->regmap, dsp->base + ADSP2_CONTROL,
				  ADSP2_MEM_ENA, ADSP2_MEM_ENA);
}

static void wm_adsp2_disable_memory(struct wm_adsp_v2 *dsp)
{
	regmap_update_bits(dsp->regmap, dsp->base + ADSP2_CONTROL,
			   ADSP2_MEM_ENA, 0);
}

static void wm_adsp2_disable_core(struct wm_adsp_v2 *dsp)
{
...
static void wm_adsp2v2_disable_core(struct wm_adsp_v2 *dsp)
{
	regmap_write(dsp->regmap, dsp->base + ADSP2_RDMA_CONFIG_1, 0);
	regmap_write(dsp->regmap, dsp->base + ADSP2_WDMA_CONFIG_1, 0);
...
{
	struct wm_adsp_v2 *dsp = container_of(work,
					   struct wm_adsp_v2,
					   boot_work);
	int ret;
...
static int wm_halo_set_rate_block(struct wm_adsp_v2 *dsp,
...
static int wm_halo_configure_mpu(struct wm_adsp_v2 *dsp, unsigned int lock_regions)
{
...
EXPORT_SYMBOL_GPL(wm_adsp2_set_dspclk_v2);
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	struct wm_adsp_v2 *dsp = &dsps[mc->shift - 1];

	ucontrol->value.integer.value[0] = dsp->preloaded;

	return 0;
}
EXPORT_SYMBOL_GPL(wm_adsp2_preloader_get_v2);
...
EXPORT_SYMBOL_GPL(wm_adsp2_preloader_put_v2);

static void wm_adsp_stop_watchdog(struct wm_adsp_v2 *dsp)
{
	regmap_update_bits(dsp->regmap, dsp->base + ADSP2_WATCHDOG,
			   ADSP2_WDT_ENA_MASK, 0);
}

static void wm_halo_stop_watchdog(struct wm_adsp_v2 *dsp)
{
...
EXPORT_SYMBOL_GPL(wm_adsp_early_event_v2);

static int wm_adsp2_start_core(struct wm_adsp_v2 *dsp)
{
	return regmap_update_bits(dsp->regmap, dsp->base + ADSP2_CONTROL,
...
static void wm_adsp2_stop_core(struct wm_adsp_v2 *dsp)
{
	regmap_update_bits(dsp->regmap, dsp->base + ADSP2_CONTROL,
...
static int wm_halo_apply_calibration(struct snd_soc_dapm_widget *w)
{
	...
	switch(dsp->fw) {
		case WM_ADSP_FW_SPK_CALI:
			...
			ret = wm_adsp_write_ctl_v2(dsp, "CAL_AMBIENT", (void *)&val, sizeof(__be32));
			if (ret)
				adsp_dbg(dsp, "%s: Write CAL_AMBIENT, ret=%d\n", __func__, ret);
			break;
		case WM_ADSP_FW_SPK_PROT:
			val = cpu_to_be32(dsp->cal_r);
			ret = wm_adsp_write_ctl_v2(dsp, "CAL_R", (void *)&val, sizeof(__be32));
			if (ret)
				adsp_dbg(dsp, "%s: Write CAL_R, ret=%d\n", __func__, ret);
			
			val = cpu_to_be32(dsp->cal_status);
			ret = wm_adsp_write_ctl_v2(dsp, "CAL_STATUS", (void *)&val, sizeof(__be32));
			if (ret)
				adsp_dbg(dsp, "%s: Write CAL_STATUS ret=%d\n", __func__, ret);
			
			val = cpu_to_be32(dsp->cal_checksum);
			ret = wm_adsp_write_ctl_v2(dsp, "CAL_CHECKSUM", (void *)&val, sizeof(__be32));
			if (ret)
				adsp_dbg(dsp, "%s: Write CAL_CHECKSUM ret=%d\n", __func__, ret);

			#if 0 // DC detection
			val = cpu_to_be32(150);
			ret = wm_adsp_write_ctl_v2(dsp, "DC_OFFSET_HOLD_TIME", (void *)&val, sizeof(__be32));
			if (ret)
...
EXPORT_SYMBOL_GPL(wm_adsp_event_v2);

static int wm_halo_start_core(struct wm_adsp_v2 *dsp)
{
	int ret;
...
static void wm_halo_stop_core(struct wm_adsp_v2 *dsp)
{
...
const struct snd_kcontrol_new wm_adsp_cal_controls_v2[] = {
...
EXPORT_SYMBOL_GPL(wm_adsp_cal_controls_v2);
...
int wm_adsp2_component_probe_v2(struct wm_adsp_v2 *dsp, struct snd_soc_component *component)
{
	...
	snd_soc_add_component_controls(component, wm_adsp_cal_controls_v2, ARRAY_SIZE(wm_adsp_cal_controls_v2));

	return 0;
}
EXPORT_SYMBOL_GPL(wm_adsp2_component_probe_v2);
...
EXPORT_SYMBOL_GPL(wm_adsp2_component_remove_v2);
...
EXPORT_SYMBOL_GPL(wm_adsp2_init_v2);
...
EXPORT_SYMBOL_GPL(wm_halo_init_v2);
...
EXPORT_SYMBOL_GPL(wm_vpu_init_v2);
...
EXPORT_SYMBOL_GPL(wm_adsp2_remove_v2);

static inline int wm_adsp_compr_attached(struct wm_adsp_compr_v2 *compr)
{
	return compr->buf != NULL;
}

static int wm_adsp_compr_attach(struct wm_adsp_compr_v2 *compr)
{
	struct wm_adsp_compr_buf_v2 *buf = NULL, *tmp;
...
static void wm_adsp_compr_detach(struct wm_adsp_compr_v2 *compr)
{
	if (!compr)
		return;
...
{
	struct wm_adsp_compr_v2 *compr, *tmp;
...
EXPORT_SYMBOL_GPL(wm_adsp_compr_open_v2);

int wm_adsp_compr_free_v2(struct snd_compr_stream *stream)
{
	struct wm_adsp_compr_v2 *compr = stream->runtime->private_data;
	struct wm_adsp_v2 *dsp = compr->dsp;
...
EXPORT_SYMBOL_GPL(wm_adsp_compr_free_v2);
...
static inline unsigned int wm_adsp_compr_frag_words(struct wm_adsp_compr_v2 *compr)
{
...
EXPORT_SYMBOL_GPL(wm_adsp_compr_set_params_v2);
...
EXPORT_SYMBOL_GPL(wm_adsp_compr_get_caps_v2);

static int wm_adsp_read_data_block(struct wm_adsp_v2 *dsp, int mem_type,
				   unsigned int mem_addr,
				   unsigned int num_words, u32 *data)
{
	struct wm_adsp_region_v2 const *mem = wm_adsp_find_region(dsp, mem_type);
	unsigned int i, reg;
...
static inline int wm_adsp_read_data_word(struct wm_adsp_v2 *dsp, int mem_type,
					 unsigned int mem_addr, u32 *data)
{
	return wm_adsp_read_data_block(dsp, mem_type, mem_addr, 1, data);
}

static int wm_adsp_write_data_word(struct wm_adsp_v2 *dsp, int mem_type,
				   unsigned int mem_addr, u32 data)
{
	struct wm_adsp_region_v2 const *mem = wm_adsp_find_region(dsp, mem_type);
	unsigned int reg;
...
static inline int wm_adsp_buffer_read(struct wm_adsp_compr_buf_v2 *buf,
...
static inline int wm_adsp_buffer_write(struct wm_adsp_compr_buf_v2 *buf,
				       unsigned int field_offset, u32 data)
{
...
static int wm_adsp_buffer_populate(struct wm_adsp_compr_buf_v2 *buf)
{
...
static void wm_adsp_buffer_clear(struct wm_adsp_compr_buf_v2 *buf)
{
...
static struct wm_adsp_compr_buf_v2 *wm_adsp_buffer_alloc(struct wm_adsp_v2 *dsp)
{
	struct wm_adsp_compr_buf_v2 *buf;
...
static int wm_adsp_buffer_parse_legacy(struct wm_adsp_v2 *dsp)
{
	struct wm_adsp_alg_region_v2 *alg_region;
	struct wm_adsp_compr_buf_v2 *buf;
...
static int wm_adsp_buffer_free(struct wm_adsp_v2 *dsp)
{
	struct wm_adsp_compr_buf_v2 *buf, *tmp;
...
static int wm_adsp_buffer_get_error(struct wm_adsp_compr_buf_v2 *buf)
{
	int ret;
...
EXPORT_SYMBOL_GPL(wm_adsp_compr_trigger_v2);

static inline int wm_adsp_buffer_size(struct wm_adsp_compr_buf_v2 *buf)
{
...
static int wm_adsp_buffer_update_avail(struct wm_adsp_compr_buf_v2 *buf)
{
...
EXPORT_SYMBOL_GPL(wm_adsp_compr_handle_irq_v2);

static int wm_adsp_buffer_reenable_irq(struct wm_adsp_compr_buf_v2 *buf)
{
...
EXPORT_SYMBOL_GPL(wm_adsp_compr_pointer_v2);

static int wm_adsp_buffer_capture_block(struct wm_adsp_compr_v2 *compr, int target)
{
	struct wm_adsp_compr_buf_v2 *buf = compr->buf;
...
static int wm_adsp_compr_read(struct wm_adsp_compr_v2 *compr,
...
int wm_adsp_compr_copy_v2(struct snd_compr_stream *stream, char __user *buf,
		       size_t count)
{
	struct wm_adsp_compr_v2 *compr = stream->runtime->private_data;
	struct wm_adsp_v2 *dsp = compr->dsp;
...
EXPORT_SYMBOL_GPL(wm_adsp_compr_copy_v2);

static void wm_adsp_fatal_error(struct wm_adsp_v2 *dsp)
{
	struct wm_adsp_compr_v2 *compr;
...
irqreturn_t wm_adsp2_bus_error_v2(struct wm_adsp_v2 *dsp)
{
	unsigned int val;
	struct regmap *regmap = dsp->regmap;
	int ret = 0;
...
EXPORT_SYMBOL_GPL(wm_adsp2_bus_error_v2);
...
EXPORT_SYMBOL_GPL(wm_halo_bus_error_v2);

irqreturn_t wm_halo_wdt_expire_v2(int irq, void *data)
{
	struct wm_adsp_v2 *dsp = data;
...
EXPORT_SYMBOL_GPL(wm_halo_wdt_expire_v2);

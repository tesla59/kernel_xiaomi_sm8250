...
static int cs35l41_enter_hibernate(struct cs35l41_v2_private *cs35l41);
static int cs35l41_exit_hibernate(struct cs35l41_v2_private *cs35l41);
static int cs35l41_restore(struct cs35l41_v2_private *cs35l41);
		       ...
		       struct snd_kcontrol *kcontrol, int event)
{
	...
	struct cs35l41_v2_private *cs35l41 =
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (cs35l41->halo_booted == false)
			wm_adsp_early_event_v2(w, kcontrol, event);
		else
	...
	case SND_SOC_DAPM_PRE_PMD:
		if (cs35l41->halo_booted == false) {
			...
			wm_adsp_early_event_v2(w, kcontrol, event);
			wm_adsp_event_v2(w, kcontrol, event);
		}
	default:
		return 0;
	}
...
static int cs35l41_set_csplmboxcmd(struct cs35l41_v2_private *cs35l41,
				   enum cs35l41_v2_cspl_mboxcmd cmd);
		       ...
		       struct snd_kcontrol *kcontrol, int event)
{
	...
	struct cs35l41_v2_private *cs35l41 =
		snd_soc_component_get_drvdata(component);
	enum cs35l41_v2_cspl_mboxcmd mboxcmd = CSPL_MBOX_CMD_NONE;
	enum cs35l41_v2_cspl_mboxstate fw_status = CSPL_MBOX_STS_RUNNING;
	int ret = 0;
...
static int cs35l41_do_fast_switch(struct cs35l41_v2_private *cs35l41)
{
	...
	wm_adsp_write_ctl_v2(&cs35l41->dsp, "CSPL_UPDATE_PARAMS_CONFIG",
...
#ifdef DEBUG
	wm_adsp_read_ctl_v2(&cs35l41->dsp, "CSPL_UPDATE_PARAMS_CONFIG",
			 data_ctl_buf, data_ctl_len * sizeof(s32));
	...
	wm_adsp_write_ctl_v2(&cs35l41->dsp, "CSPL_COMMAND", &cmd_ctl, sizeof(s32));

	/* Verify CSPL COMMAND */
	for (i = 0; i < 5; i++) {
		wm_adsp_read_ctl_v2(&cs35l41->dsp, "CSPL_STATE", &st_ctl,
				 sizeof(s32));
...
static bool cs35l41_is_csplmboxsts_correct(enum cs35l41_v2_cspl_mboxcmd cmd,
					   enum cs35l41_v2_cspl_mboxstate sts)
{
	switch (cmd) {
...
static void cs35l41_abort_ramp(struct cs35l41_v2_private *cs35l41)
{
...
static void cs35l41_set_vol(int vol, struct cs35l41_v2_private *cs35l41)
{
	unsigned int val;
	int ret;
...
static int cs35l41_vol_ramp0(struct cs35l41_v2_private *cs35l41,
...
{
	struct cs35l41_v2_vol_ctl *vol_ctl;
	struct cs35l41_v2_private *cs35l41;
	long final_x_knee, final_x_end;
	long init_y_knee, init_y_end, final_y_end;
	int ret = 0;

	vol_ctl = container_of(wk, struct cs35l41_v2_vol_ctl, ramp_work);
	cs35l41 = container_of(vol_ctl, struct cs35l41_v2_private, vol_ctl);
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (cs35l41->dsp.booted) {
	  wm_adsp_read_ctl_v2(&cs35l41->dsp, "CH_BAL", (void *)&val, sizeof(__be32));
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (cs35l41->dsp.booted) {
	  ...
	  val = cpu_to_be32(val);
	  wm_adsp_write_ctl_v2(&cs35l41->dsp, "CH_BAL", (void *)&val, sizeof(__be32));
	}
...
static const struct cs35l41_v2_otp_map_element_t *
	cs35l41_find_otp_map(u32 otp_id, u32 devid_otp)
{
	int i;

	for (i = 0; i < cs35l41_v2_otp_maps.len; i++) {
		if (cs35l41_v2_otp_maps.map[i].id == otp_id &&
		    cs35l41_v2_otp_maps.map[i].devid_otp == devid_otp)
			return &cs35l41_v2_otp_maps.map[i];
	}
...
{
	struct cs35l41_v2_private *cs35l41 = data;
	u32 *otp_mem;
	int i;
	int bit_offset, word_offset;
	unsigned int bit_sum = 8;
	u32 otp_val, otp_id_reg;
	const struct cs35l41_v2_otp_map_element_t *otp_map_match;
	const struct cs35l41_v2_otp_packed_element_t *otp_map;
	int ret;
...
static void cs35l41_hibernate_work(struct work_struct *work)
{
	...
	struct cs35l41_v2_private *cs35l41 =
		container_of(dwork, struct cs35l41_v2_private, hb_work);
...
static bool cs35l41_need_auto_vol_ramp(struct cs35l41_v2_private *cs35l41)
{
	bool ramp = false;
...
static const struct wm_adsp_region_v2 cs35l41_dsp1_regions[] = {
...
static int cs35l41_boost_config(struct cs35l41_v2_private *cs35l41,
...
static int cs35l41_set_pdata(struct cs35l41_v2_private *cs35l41)
{
	struct classh_cfg_v2 *classh = &cs35l41->pdata.classh_config;
	int ret;
	u32 devid_otp;
...
static int cs35l41_component_probe(struct snd_soc_component *component)
{
	...
	if (!(cs35l41->skip_codec_probe)) {
		wm_adsp2_component_probe_v2(&cs35l41->dsp, component);
...
static int cs35l41_irq_gpio_config(struct cs35l41_v2_private *cs35l41)
{
	struct irq_cfg_v2 *irq_gpio_cfg1 = &cs35l41->pdata.irq_config1;
	struct irq_cfg_v2 *irq_gpio_cfg2 = &cs35l41->pdata.irq_config2;
...
static void cs35l41_component_remove(struct snd_soc_component *component)
{
	...
	wm_adsp2_component_remove_v2(&cs35l41->dsp, component);
}
...
static int cs35l41_handle_of_data(struct device *dev,
				  struct cs35l41_v2_platform_data *pdata,
				  struct cs35l41_v2_private *cs35l41)
{
	...
	struct classh_cfg_v2 *classh_config = &pdata->classh_config;
	struct irq_cfg_v2 *irq_gpio1_config = &pdata->irq_config1;
	struct irq_cfg_v2 *irq_gpio2_config = &pdata->irq_config2;
	unsigned int i;
...
static int cs35l41_dsp_init(struct cs35l41_v2_private *cs35l41)
{
	...
	ret = wm_halo_init_v2(dsp, &cs35l41->rate_lock);
...
static int cs35l41_enter_hibernate(struct cs35l41_v2_private *cs35l41)
{
	...
	for (i = 0; i < CS35L41_CTRL_CACHE_SIZE; i++)
		regmap_read(cs35l41->regmap, cs35l41_v2_ctl_cache_regs[i],
			    &cs35l41->ctl_cache[i]);
...
static int cs35l41_wait_for_pwrmgt_sts(struct cs35l41_v2_private *cs35l41)
{
	int i, ret = 0;
...
static int cs35l41_exit_hibernate(struct cs35l41_v2_private *cs35l41)
{
	...
	for (i = 0; i < CS35L41_TRIM_CACHE_REGIONS; i++) {
		regmap_raw_write(cs35l41->regmap,
				cs35l41_v2_trim_cache_regs[i].reg,
				p_trim_data, cs35l41_v2_trim_cache_regs[i].size *
				sizeof(u32));
		p_trim_data += cs35l41_v2_trim_cache_regs[i].size;
	}
...
static int cs35l41_restore(struct cs35l41_v2_private *cs35l41)
{
	...
	/* These values are already cached in cs35l41_v2_private struct */
...
static int cs35l41_preset_sample_rate(struct cs35l41_v2_private *cs35l41, int rate)
{
	int i;
...
int cs35l41_v2_probe(struct cs35l41_v2_private *cs35l41,
				struct cs35l41_v2_platform_data *pdata)
{
	int ret;
	...
	cs35l41->vol_ctl.ramp_wq =
		create_singlethread_workqueue("cs35l41_v2_ramp");
	...
	cs35l41->wq = create_singlethread_workqueue("cs35l41_v2");
	if (cs35l41->wq == NULL) {
		ret = -ENOMEM;
		goto err;
	}
